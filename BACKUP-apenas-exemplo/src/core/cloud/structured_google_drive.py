...
try:
    ...
    class MockGoogleAPI:
        ...
        @classmethod
        def run_local_server(cls, *args, **kwargs):
            return cls()
...
class StructuredGoogleDriveManager:
    ...
    def _is_google_drive_folder(self, path: Path) -> bool:
        ...
        try:
            ...
            indicators = [path / ".tmp.drivedownload", path / "desktop.ini"]
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    ...
                    if credentials_file.exists():
                        ...
                        print(
                            "   ðŸ“ Para habilitar API: https://developers.google.com/drive/api/quickstart/python"
                        )
                        return False
        ...
        try:
            ...
            for folder_name, description in subfolders.items():
                ...
                if not readme_path.exists():
                    ...
                    readme_content += "Pasta criada automaticamente pelo JARVIS.\n"
    ...
    def sync_models_to_drive(self, models_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            metadata = {
                ...
                "device_id": (
                    self.microsoft_identifier.device_fingerprint.device_id
                    if self.microsoft_identifier.device_fingerprint
                    else "unknown"
                ),
                "user_account": (
                    self.microsoft_identifier.microsoft_account.account_email
                    if self.microsoft_identifier.microsoft_account
                    else "unknown"
                ),
        ...
        try:
            ...
            for config_file in config_files:
                if config_file.exists():
                    ...
                    device_id = (
                        self.microsoft_identifier.device_fingerprint.device_id[:8]
                        if self.microsoft_identifier.device_fingerprint
                        else "unknown"
                    )
    ...
    def backup_system_to_drive(self) -> bool:
        ...
        try:
            ...
            backups_folder = (
                self.local_drive_path / str(self.user_folder_name) / "backups"
            )
            ...
            metadata = {
                ...
                "compression_ratio": round(
                    (1 - backup_file.stat().st_size / max(1, total_size)) * 100, 2
                ),
            }
            ...
            print(
                f"   ðŸ“¦ Compactado para {backup_file.stat().st_size/1024/1024:.1f} MB"
            )
    ...
    def consolidate_data_from_devices(self) -> Dict[str, Any]:
        ...
        try:
                ...
                for model_file in models_folder.glob("*.json"):
                    try:
                        ...
                        if "model_name" in model_data:
                            ...
                            device_id = model_data.get("metadata", {}).get(
                                "device_id", "unknown"
                            )
                            ...
                            consolidated_data["models"][model_name].append(
                                {
                                    ...
                                    "timestamp": model_data.get("metadata", {}).get(
                                        "sync_timestamp"
                                    ),
                                }
                            )
                ...
                for config_file in configs_folder.glob("*.json"):
                    if "sync_info" in config_file.name:
                        try:
                            ...
                            consolidated_data["configs"][device_id].extend(
                                sync_data.get("synced_files", [])
                            )
                        except Exception:
                            continue
            ...
            consolidated_file = (
                sync_folder / f"consolidated_data_{int(time.time())}.json"
            )
            with open(consolidated_file, "w", encoding="utf-8") as f:
                json.dump(consolidated_data, f, indent=2, ensure_ascii=False)

            print(
                f"âœ… Dados consolidados de {len(consolidated_data['models'])} modelos"
            )
            print(
                f"   ðŸ“± {len(consolidated_data['configs'])} dispositivos encontrados"
            )
    ...
    def _start_auto_sync(self):
        ...
        print(
            f"ðŸ”„ SincronizaÃ§Ã£o automÃ¡tica iniciada (intervalo: {self.sync_interval_minutes} minutos)"
        )
        ...
        while not self.stop_sync.wait(self.sync_interval_minutes * 60):
            try:
                ...
                if (
                    ...
                    or datetime.now() - self.last_sync > timedelta(hours=4)
                ):
