...
from src.learning.semantic_feedback import get_semantic_analyzer, process_interaction_feedback
...
def simulate_learning_scenario():
    ...
    positive_interactions = [
        ("Explique o que é Machine Learning", "Machine Learning é um campo da inteligência artificial que permite aos sistemas aprenderem automaticamente com dados, sem serem explicitamente programados para cada tarefa específica."),
        ("Como funciona o aprendizado supervisionado?", "No aprendizado supervisionado, o algoritmo aprende com um conjunto de dados rotulado, onde cada exemplo tem uma entrada e uma saída conhecida. O objetivo é aprender uma função que mapeie entradas para saídas."),
        ("Quais são os tipos de Machine Learning?", "Existem três tipos principais: supervisionado (com rótulos), não supervisionado (sem rótulos) e por reforço (aprendizado através de recompensas e punições).")
    ]
    ...
    dissonant_interactions = [
        ("Como otimizar um modelo de ML?", "Não sei, pergunte ao Google."),  # Resposta ruim
        ("Qual a diferença entre overfitting e underfitting?", "São a mesma coisa."),  # Resposta incorreta
    ...
    improved_interactions = [
        ("Como otimizar um modelo de ML?", "Para otimizar um modelo de Machine Learning, você pode usar técnicas como validação cruzada, ajuste de hiperparâmetros com grid search ou random search, regularização para evitar overfitting, e ensemble methods como bagging ou boosting."),
        ("Qual a diferença entre overfitting e underfitting?", "Overfitting ocorre quando o modelo se ajusta demais aos dados de treinamento, perdendo capacidade de generalização. Underfitting acontece quando o modelo é muito simples para capturar os padrões dos dados."),
        ("Explique gradient descent", "Gradient descent é um algoritmo de otimização usado para minimizar a função de custo em machine learning. Ele funciona calculando o gradiente da função de erro e atualizando os parâmetros na direção oposta ao gradiente.")
    ]
    ...
    print("O JARVIS agora aprende com suas próprias falhas e se adapta automaticamente.")
