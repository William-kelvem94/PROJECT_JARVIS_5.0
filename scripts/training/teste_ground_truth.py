...
def test_truth_validator_standalone():
    ...
    try:
        ...
        print(f"   Verdade consolidada: {result1.get('consolidated_truth', '')[:200]}...")
        ...
        comparison = validator.compare_with_auto_critique(jarvis_error, web_truth, auto_critique)
...
def test_semantic_feedback_integration():
    ...
    try:
        ...
        for interaction in test_interactions:
            result = analyzer.process_interaction(interaction["user"], interaction["ai"])
            processed_interactions.append(result)
            logger.info(f"📊 Interação processada - Confiança: {result.confidence_score:.2f}, Dissonância: {result.dissonance_detected}")

        # Verificar se dissonância foi detectada
        dissonant_count = sum(1 for i in processed_interactions if i.dissonance_detected)
        print(f"\n🎯 Dissonâncias detectadas: {dissonant_count}/{len(test_interactions)}")

        # Forçar trigger de auto-reparo (simulando baixa confiança)
        analyzer.confidence_metrics.average_confidence = 0.3  # Forçar abaixo do threshold
...
def run_all_tests():
    ...
    print(f"\n{'='*60}")
    print(f"📊 RESULTADO DOS TESTES - GROUND TRUTH VALIDATION")
    print(f"{'='*60}")
