...
problematic_modules = [
    'mediapipe', 'tensorflow', 'cv2', 'face_recognition', 'dlib',
    'pygame', 'pycaw', 'win32api', 'winsound', 'pyaudio'
]
...
class CoreIntegrationTest(unittest.TestCase):
        ...
        try:
            ...
            with patch('src.core.iot.iot_manager.config') as mock_config:
                ...
                with patch('src.core.iot.iot_manager.requests.post') as mock_post:
                    ...
                    result = iot_configured.control_device("light.living_room", "turn_on")
                    ...
                    expected_url = "http://192.168.1.100:8123/api/services/light/turn_on"
                    self.assertEqual(args[0], expected_url)
                    
                    self.assertIn('Authorization', kwargs['headers'])
                    self.assertEqual(kwargs['headers']['Authorization'], 'Bearer test_secure_token_123')
            ...
            with patch('src.core.orchestrator.SecurityManager') as mock_security:
                with patch('src.core.orchestrator.IOTManager') as mock_iot:
                    with patch('src.core.orchestrator.FallbackSystem') as mock_fallback:
                        ...
                        with patch.object(orchestrator, '_init_interface_orchestration'):
    ...
    def test_init_files_improvements(self):
        ...
        try:
            ...
            self.assertGreater(len(content.strip()), 10, "core/__init__.py não deve estar vazio")
            ...
            for submodule in submodules:
                ...
                if init_path.exists():
                    ...
                    self.assertNotEqual(content.strip(), "# clean", f"{submodule}/__init__.py foi melhorado")
                    if '"""' in content:
                        print(f"   ✅ {submodule}/__init__.py melhorado e documentado")
                    else:
                        print(f"   ⚠️ {submodule}/__init__.py melhorado mas sem documentação")
                else:
            ...
            try:
                ...
                if hasattr(src.core.security, '__all__'):
                    print(f"   ✅ src.core.security.__all__ = {src.core.security.__all__}")
                ...
                with patch('src.core.orchestrator.IOTManager') as mock_iot:
                    with patch('src.core.orchestrator.FallbackSystem') as mock_fallback:
                        with patch('src.core.iot.iot_manager.config') as mock_config:
                            ...
                            mock_security_instance = Mock()
                            mock_security_instance.validate_path_access.return_value = True
                            mock_security_instance.validate_web_request.return_value = True
                            mock_security.return_value = mock_security_instance
