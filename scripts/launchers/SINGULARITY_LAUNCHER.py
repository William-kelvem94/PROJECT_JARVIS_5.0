...
>>>>>>> dev-new-version
    ...
    logger.warning(
        "[WARN] [INFRA] Ollama nao detectado. Iniciando Protocolo de Instalacao Automatica..."
    )
    ...
    try:
        ...
        urllib.request.urlretrieve(
            OLLAMA_INSTALLER_URL, installer_path, report_progress
        )
        print("\rProgresso: 100.0%")

        logger.info(
            "🚀 Executando instalador... Por favor, siga as instruções na tela."
        )
        # Executa o instalador e aguarda
        install_process = subprocess.Popen([installer_path])
...
>>>>>>> Stashed changes
            ...
            logger.warning(
                "⚠️ Caminho padrão de instalação não encontrado. O PATH pode estar desatualizado."
            )
        ...
        if shutil.which("ollama"):
            ...
            logger.error(
                "[ERR] [ERRO] Falha na verificacao pos-instalacao. O comando 'ollama' ainda nao foi encontrado."
            )
            sys.exit(1)
...
def ensure_ollama_running():
    ...
    try:
        ...
        logger.warning(
            "[WARN] [INFRA] Servidor Neural Offline. Inicializando motor local..."
        )
        ...
        try:
            ...
            logger.error(
                "[ERR] [ERRO] O servidor Ollama demorou demais para responder."
            )
            sys.exit(1)
...
def main():
    ...
    print("🌐 Modo: Hibrido (Local + Elite Search)")
    print("🔒 Identificação: Biometric + Identity Guard")
    print("🚀 Status: Tronco Encefalico Ativo")
    print("=" * 80)
    ...
    try:
        ...
        result = subprocess.run(
            ...
            text=False,
            timeout=10,
        )  # Timeout reduzido e text=False para evitar crash

        if result.returncode == 0:
            stdout_text = result.stdout.decode("utf-8", errors="replace")
            lines = stdout_text.strip().split("\n")
            account_line = next(
                (line for line in lines if line.startswith("ACCOUNT:")), None
            )
            device_line = next(
                (line for line in lines if line.startswith("DEVICE:")), None
            )
            ...
            if account_line and device_line:
                ...
                if account != "None" and device != "None":
                    ...
                    print("   [WARN] Identificacao parcial - continuando...")
            else:
                print("   [WARN] Resposta inesperada - continuando...")
        else:
            print("   [WARN] Falha na deteccao - continuando sem identificacao...")

    except subprocess.TimeoutExpired:
        print(
            "   [WARN] Timeout na identificacao - pulando para inicializacao rapida..."
        )
    except Exception as e:
        print(
            f"   [WARN] Erro na identificacao: {e} - pulando para inicializacao rapida..."
        )
    ...
    if exit_code == 123:  # Código personalizado para Rebirth se necessário
        ...
        print(
            f"[WARN] [AVISO] JARVIS encerrou com codigo {exit_code}. Verifique os logs."
        )
        print("\n" + "=" * 80)
        ...
        print(
            "   • Execute: pip install --upgrade torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu"
        )
        ...
        print(
            "   * Execute: python -c \"import torch; print('Torch OK:', torch.__version__)\""
        )
        print()
