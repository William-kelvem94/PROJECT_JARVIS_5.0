...
class TotalInstaller:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    self.project_root / "logs" / "total_installer.log", encoding="utf-8"
                ),
    ...
    def install_pytorch_correct(self):
        ...
        if self._check_cuda():
            self.logger.info(
                "[GPU] GPU detectada! Instalando PyTorch 2.4+ CUDA 12.1..."
            )
            torch_cmd = [
                sys.executable,
                "-m",
            ...
            self.logger.info(
                "[SYS] CPU detectada. Instalando PyTorch 2.4.1+cpu (Stable)..."
            )
            torch_cmd = [
                sys.executable,
                "-m",
    ...
    def install_face_recognition_complete(self):
        ...
        try:
            ...
            self.logger.error(
                f"[ERR] Falha ao instalar modelos via git: {e}. Tentando via pip padrao..."
            )
            try:
                self.run_command(
    ...
    def install_all(self):
        ...
        if dlib_wheel and os.path.exists(dlib_wheel):
            ...
            try:
                self.run_command(
                    [sys.executable, "-m", "pip", "install", dlib_wheel],
                    skip_capture=True,
                )
            except Exception as e:
            ...
            self.logger.warning(
                "[WARN] Nao foi possivel baixar um wheel compativel. Tentando dlib-bin..."
            )
            try:
                self.run_command(
        ...
        numpy_conflicting = [
            "optimum-intel[openvino,nncf]>=1.18.0",
