...
def validate():
    ...
    try:
        ...
        print(
            f"✅ PyTorch {torch.__version__} pronto (CUDA: {torch.cuda.is_available()})"
        )
    ...
    try:
        ...
        print("✅ TTS - Voice Cloning XTTS-v2")
    except ImportError:
    ...
    try:
        ...
        print("✅ noisereduce - Audio Enhancement (+20% STT accuracy)")
    except ImportError:
    ...
    if missing_libs:
        ...
        for lib in missing_libs:
            try:
                ...
                is_tts = "TTS" in lib
                timeout_duration = (
                    120 if is_tts else 180
                )  # 2min para TTS, 3min para outros

                if is_tts:
                    print(
                        f"⏳ Instalando {lib} (pode falhar sem Visual C++ Build Tools, timeout: 2min)..."
                    )
                else:
                ...
                if result.returncode == 0:
                    ...
                    if is_tts:
                        print(
                            f"   ⚠️  {lib} falhou (esperado sem Visual C++ Build Tools)"
                        )
                        print(
                        ...
                        stderr_short = (
                            result.stderr[:300] if result.stderr else "Sem detalhes"
                        )
                ...
                if is_tts:
                    ...
                    print(
                        "      TTS compilação falhou. Sistema usará Edge-TTS/pyttsx3."
                    )
                else:
    ...
    models = ["models/yolov8n.pt", "models/hand_landmarker.task"]
    missing = []
    for m in models:
    ...
    if missing:
        ...
        choice = (
            input("Gostaria de baixar os modelos necessários agora? (S/N): ")
            .strip()
            .lower()
        )
        if choice == "s":
            try:
                subprocess.run(
                    [sys.executable, "scripts/install/download_models.py"], check=True
                )
