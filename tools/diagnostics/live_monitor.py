...
while True:
    ...
    for p in psutil.process_iter(['pid','name','username','memory_info','cpu_percent']):
        try:
            ...
            procs.append((rss, info['pid'], info.get('name',''), info.get('cpu_percent',0.0)))
        except Exception:
            continue
        ...
        try:
            ...
            if comps:
                ...
                for cname, comp in comps.items():
                    ...
                    print(f"  {cname:30} status={comp.status.value:8} last_hb={hb_age} mem={comp.memory_usage_mb:.1f}MB cpu={comp.cpu_usage_percent:.1f}%")
            else:
