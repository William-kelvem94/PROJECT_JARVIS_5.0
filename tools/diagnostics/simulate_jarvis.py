...
def run_test_suite():
    ...
    try:
        ...
        res = neural_dreaming.start_dream("Programação Quântica", duration_min=1, focus_mode=False)
        if res:
            print("  ✅ Protocolo Dreaming iniciado em background.")
            time.sleep(1)
            if neural_dreaming.is_dreaming:
                print(f"  ✅ Estado Dreaming: {neural_dreaming.is_dreaming} (Tópico: {neural_dreaming.current_topic})")
    ...
    try:
        ...
        class MockVC:
            def speak(self, text, mode='online'): print(f"  🗣️ Jarvis: {text}")
