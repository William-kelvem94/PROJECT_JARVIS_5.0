...
def validate_ai_agent_logic():
    ...
    try:
        ...
        sys.modules['src.core.local_brain'].local_brain.generate_response = MagicMock(return_value="Local Mock Response")
        ...
        with patch('src.core.voice_controller.voice_controller.check_internet', return_value=True), \
             patch('src.core.screen_capture.screen_capture.capture_fullscreen', return_value="path/to/img.png"):
...
if __name__ == "__main__":
    ...
    if not validate_core_imports(): success = False
    if not validate_voice_logic(): success = False
    if not validate_ai_agent_logic(): success = False
