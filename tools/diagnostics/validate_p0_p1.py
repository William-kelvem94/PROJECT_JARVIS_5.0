...
def test_wake_word_detection():
    ...
    try:
        ...
        logger.warning(
            "⚠️  P0.1: Wake Word Detection - NOT INSTALLED (pip install pvporcupine)"
        )
        return False
...
def test_semantic_caching():
    ...
    try:
        ...
        if hasattr(vision, "ocr_cache"):
            ...
            logger.info(
                f"   Cache stats: {vision.ocr_cache_hits} hits, {vision.ocr_cache_misses} misses"
            )
            return True
        else:
...
def test_noise_reduction():
    ...
    try:
        ...
        logger.warning(
            "⚠️  P1.2: Noise Reduction - NOT INSTALLED (pip install noisereduce)"
        )
        return False
...
def test_rag_upgrade_jina():
    ...
    try:
        ...
        if "jina" in source.lower() or "JINA" in source:
            ...
            logger.info(
                "⚠️  P1.6: RAG Upgrade - Using default embeddings (not critical)"
            )
