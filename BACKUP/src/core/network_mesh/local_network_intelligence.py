...
class LocalNetworkIntelligence:
    ...
    def __init__(self, jarvis_core_path: str):
        ...
        print("ðŸ  Local Network Intelligence inicializado")
        print(
            f"   ðŸ“± Dispositivo: {self.local_device.device_name} ({self.local_device.role.value})"
        )
    ...
    def _detect_local_device(self) -> LocalJarvisDevice:
        ...
        device_id = hashlib.sha256(
            f"{user_account}:{hardware_info}".encode()
        ).hexdigest()[:16]
    ...
    def _get_hardware_fingerprint(self) -> str:
        ...
        try:
            ...
            return hashlib.sha256(
                f"{platform.node()}:{time.time()}".encode()
            ).hexdigest()[:16]
    ...
    def _detect_system_capabilities(self) -> List[str]:
        ...
        # Webcam/Camera (FIX: Avoid synchronous cv2.VideoCapture(0) as it hangs boot)
        try:
            from src.core.config.system_manifest import system_manifest
            # If manifest says we have vision, or if we are in production, 
            # we assume the capability exists without probing the hardware here.
            if getattr(system_manifest.vision, "enabled", True):
    ...
    async def _register_device_in_cloud(self):
        ...
        device_file = (
            self.jarvis_cloud_folder / "devices" / f"{self.local_device.device_id}.json"
        )
    ...
    async def _upload_pending_intelligence(self):
        ...
        for packet in self.sync_queue.copy():
            try:
                ...
                folder = self.jarvis_cloud_folder / folder_map.get(
                    packet.packet_type, "learning"
                )
                timestamp = packet.timestamp.strftime("%Y%m%d_%H%M%S")
                filename = (
                    f"{packet.source_device}_{timestamp}_{packet.packet_type}.json"
                )
    ...
    async def share_memory(
        self, memory_type: str, memory_data: Dict, importance: int = 5
    ):
        """ðŸ“¤ COMPARTILHA MEMÃ“RIA COM REDE"""
        packet = IntelligencePacket(
            packet_id=hashlib.sha256(
                f"{time.time()}:{memory_type}".encode()
            ).hexdigest()[:16],
            source_device=self.local_device.device_id,
            packet_type="memory",
            content={"memory_type": memory_type, "data": memory_data},
    ...
    async def share_learning(
        self, learning_type: str, learning_data: Dict, importance: int = 7
    ):
        """ðŸŽ“ COMPARTILHA APRENDIZADO COM REDE"""
        packet = IntelligencePacket(
            packet_id=hashlib.sha256(
                f"{time.time()}:{learning_type}".encode()
            ).hexdigest()[:16],
            source_device=self.local_device.device_id,
            packet_type="learning",
            content={"learning_type": learning_type, "data": learning_data},
            timestamp=datetime.now(),
            importance_level=importance,
            sync_encrypted=False,
        )
    ...
    async def emergency_broadcast(self, emergency_type: str, emergency_data: Dict):
        ...
        packet = IntelligencePacket(
            packet_id=hashlib.sha256(f"{time.time()}:EMERGENCY".encode()).hexdigest()[
                :16
            ],
            source_device=self.local_device.device_id,
...
async def test_local_network_intelligence():
    ...
    print(
        f"   â˜ï¸ Google Drive: {'âœ…' if status['google_drive_connected'] else 'âŒ'}"
    )
    ...
    await network.share_memory(
        "user_preference",
        {"language": "pt-BR", "voice_speed": "normal", "preferred_ai_model": "local"},
        importance=6,
    )

    # Compartilhar um aprendizado
    await network.share_learning(
        "voice_recognition",
        {
