    ...
    def execute_action(self, action: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if action_name == "read_file":
                ...
                if self.security and not self.security.validate_path_access(
                    path, "read"
                ):
                ...
                with open(path, "r", encoding="utf-8", errors="ignore") as f:
                    return {
                        "status": "success",
                        "action": action_name,
                        "result": f.read(MAX_FILE_READ_SIZE),
                    }
            ...
            if action_name in ["minimize_window", "maximize_window", "focus_window", "close_window"]:
                if not self.gui:
                    return {"status": "error", "error": "GUI controller not available"}
                target_title = action_dict.get("title", "")
                win_action = action_name.split("_")[0]
                success = self.gui.manage_window(target_title, win_action)
                return {"status": "success" if success else "error", "action": action_name, "target": target_title}

            if action_name in ["lock_system", "restart_system", "shutdown_system"]:
                if not self.gui:
                    return {"status": "error", "error": "System controller not available"}
                sys_action = action_name.split("_")[0]
                # Security check (requires HITL or specific permission)
                logger.warning(f"⚠️ Critical System Action Requested: {action_name}")
                success = self.gui.system_power(sys_action)
                return {"status": "success" if success else "error", "action": action_name}

            if action_name == "get_active_window":
                if not self.gui:
                    return {"status": "error", "error": "GUI controller not available"}
                title = self.gui.get_active_window()
                return {"status": "success", "result": title}

            # Add more handlers from the old complex executor as needed

            return {
                "status": "error",
                "error": f"Handler for '{action_name}' not implemented in unified executor",
            }
