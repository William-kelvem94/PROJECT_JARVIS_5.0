...
try:
    ...
    logger.info("✅ Optimum-Intel (OpenVINO) detectado e pronto para o Cérebro Local")
except ImportError:
    OPENVINO_AVAILABLE = False
    OVModelForCausalLM = None
    logger.warning("⚠️ Optimum-Intel não encontrado. LLM rodará em CPU lenta.")
...
class LocalBrain:
    ...
    def load_async(self):
        ...
        try:
            ...
            if mem.percent > 85.0:
                logger.warning(
                    f"⚠️ Memory Guard: System RAM at {mem.percent}%. Skipping LocalBrain auto-load to prevent freeze."
                )
                return
        except ImportError:
            pass
        ...
        threading.Thread(
            target=self._load_model_background, daemon=True, name="Stark-Neural-Loader"
        ).start()
        ...
        try:
            with self._load_lock:
                ...
                if not TRANSFORMERS_AVAILABLE:
                    raise ImportError(
                        "Transformers library not available for LocalBrain"
                    )
                ...
                if (
                    ...
                    try:
                        ...
                        logger.warning(
                            f"⚠️ GPU indisponível ou falhou ({e}). Tentando OpenVINO CPU..."
                        )
                        try:
                            self.model = OVModelForCausalLM.from_pretrained(
                                self.model_id,
                            ...
                            logger.error(
                                f"❌ Falha total no OpenVINO: {e2}. Ativando fallback PyTorch."
                            )
                            self.model = None
    ...
    def generate_response(
        self, prompt: str, system_prompt: str = "", max_new_tokens: int = 128
    ) -> str:
