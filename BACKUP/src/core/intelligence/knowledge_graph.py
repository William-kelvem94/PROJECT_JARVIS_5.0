...
class KnowledgeGraph:
    ...
    def add_concept(
        ...
        if concept not in self.graph:
            self.graph.add_node(
                concept, type=concept_type, metadata=metadata or {}, count=1
            )
    ...
    def add_relation(
        ...
        if self.graph.has_edge(concept_a, concept_b):
            ...
            self.graph.add_edge(
                concept_a, concept_b, relation=relation_type, weight=weight
            )
    ...
    def add_from_text(self, text: str, source: str = "user"):
        ...
        for entity_type, entity_list in entities.items():
            for entity in entity_list:
                self.add_concept(
                    entity, concept_type=entity_type, metadata={"source": source}
                )
        ...
        for i, concept_a in enumerate(all_concepts):
            for concept_b in all_concepts[i + 1 :]:
                if concept_a != concept_b:
                    self.add_relation(
                        concept_a, concept_b, relation_type="co_occurs", weight=0.5
                    )
    ...
    def get_related_concepts(
        self, concept: str, max_depth: int = 2, limit: int = 10
    ...
    def get_top_concepts(
        self, n: int = 20, concept_type: Optional[str] = None
    ) -> List[Tuple[str, int]]:
        """
    ...
    def visualize_subgraph(self, center_concept: str, depth: int = 2):
        ...
        try:
            ...
            nx.draw_networkx_edges(
                subgraph, pos, edge_color="#CCCCCC", arrows=True, arrowsize=20
            )
