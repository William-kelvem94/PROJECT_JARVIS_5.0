...
class VoiceController:
    ...
    Levels:
    0. ElevenLabs (Elite Cloud - Primary)
    1. Local XTTS-v2 (Neural Cloning)
    2. Edge-TTS (Cloud Neural - Fast)
    3. pyttsx3 (Offline Fallback - Robotic)
    ...
    def _synthesize_and_speak(self, text: str, cache_path: Path, wait: bool):
        """Lógica de cascata de síntese (ElevenLabs -> XTTS -> Edge -> pyttsx3)."""
        success = False

        # Nível 0: ElevenLabs (Elite Cloud)
        api_key = os.getenv("ELEVEN_LABS_API_KEY")
        if ELEVENLABS_AVAILABLE and api_key:
            try:
                logger.info("💎 [LEVEL 0] ElevenLabs Synthesis (Elite Mode)")
                set_api_key(api_key)
                audio = generate(
                    text=text,
                    voice="Adam", # Default JARVIS-like voice
        ...
        # Nível 1: XTTS Local (Priority Fallback)
        if not success and self._xtts_ready and self.reference_wav.exists():
            try:
