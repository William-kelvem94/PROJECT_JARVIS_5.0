...
class DataAnalyzer:
    ...
    def _load_nlp_model(self):
        ...
        try:
            ...
            logger.warning(
                "Modelo spaCy nÃ£o encontrado. Alguns recursos estarÃ£o limitados."
            )
            try:
    ...
    def _load_document_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "invoice": {
                ...
                "patterns": [
                    r"fatura\s+\w+",
                    r"boleto\s+bancÃ¡rio",
                    r"conta\s+de\s+luz|Ã¡gua|telefone",
                ],
    ...
    def analyze_text(
        self, text: str, capture_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            avg_confidence = (
                sum(item.get("confidence", 0) for item in extracted_data)
                / len(extracted_data)
                if extracted_data
                else 0
            )
            ...
            logger.info(
                f"AnÃ¡lise concluÃ­da: {len(extracted_data)} dados extraÃ­dos, {len(categories)} categorias"
            )
            return result
    ...
    def _calculate_basic_sentiment(self, doc) -> float:
        ...
        positive_count = sum(
            1 for token in doc if token.lemma_.lower() in positive_words
        )
        negative_count = sum(
            1 for token in doc if token.lemma_.lower() in negative_words
        )
    ...
    def _save_analysis_results(self, capture_id: int, analysis_result: Dict[str, Any]):
        ...
        try:
            ...
            for data_item in analysis_result.get("extracted_data", []):
                ...
                db_manager.execute_in_session(
                    lambda session: session.add(extracted_data)
                )
    ...
    def get_data_summary(self, capture_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "confidence_avg": (
                    sum(item.confidence for item in extracted) / len(extracted)
                    if extracted
                    else 0
                ),
            }
        ...
        try:
            ...
            for correction in feedback_data.get("corrections", []):
                ...
                if field_name and correct_value and incorrect_value:
                    ...
                    logger.info(
                        f"Aprendizado: {field_name} - '{incorrect_value}' -> '{correct_value}'"
                    )
