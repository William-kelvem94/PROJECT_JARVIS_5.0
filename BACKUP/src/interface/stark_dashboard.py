...
class StarkDashboard(QMainWindow):
    ...
    def __init__(self, jarvis_core=None):
        ...
        # Stylesheet (Stark Luxury Glass Theme - v3.0 Singularity)
        self.setStyleSheet("""
            QMainWindow { 
            ...
            QTabWidget::pane { 
                ...
                background: rgba(11, 15, 25, 180); 
                border-radius: 15px;
                margin-top: -1px;
            }
            
            QTabBar::tab { 
                background: rgba(5, 10, 16, 220); 
                color: #8899a6; 
                padding: 14px 30px; 
                border: 1px solid rgba(31, 41, 58, 200); 
                border-bottom: none; 
            ...
            QTabBar::tab:selected { 
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1, 
                    stop:0 rgba(0, 195, 255, 50), 
                    stop:0.5 rgba(0, 195, 255, 20), 
                    stop:1 rgba(11, 15, 25, 255));
                color: #00c3ff; 
            ...
            QPushButton { 
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, 
                    stop:0 rgba(31, 41, 58, 150), 
                    stop:1 rgba(11, 15, 25, 200));
                color: #00c3ff; 
            ...
            QPushButton:hover { 
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, 
                    stop:0 rgba(0, 195, 255, 80), 
                    stop:1 rgba(11, 15, 25, 220));
    ...
    def setup_sentinel_tab(self):
        ...
        self.camera_feed_label.setStyleSheet(
            "background-color: #000; color: #00c3ff; font-family: Consolas;"
        )
    ...
    def setup_cognitive_tab(self):
        ...
        self.study_status_label.setStyleSheet(
            "font-size: 14px; font-weight: bold; color: #8899a6;"
        )
        ...
        self.curiosity_list_widget.setStyleSheet(
            "background-color: #0b0f19; padding: 10px; border: 1px dashed #1f293a; color: #e0e0e0;"
        )
    ...
    def _force_study_cycle(self):
        ...
        try:
            ...
            if curiosity_engine:
                ...
                threading.Thread(
                    target=curiosity_engine.run_study_cycle, daemon=True
                ).start()
                self.status_bar.showMessage(
                    "🚀 Ciclo de Estudo de Emergência iniciado!", 5000
                )
            else:
    ...
    def _handle_learning_status(self, topic, is_studying):
        ...
        if is_studying:
            ...
            self.study_status_label.setStyleSheet(
                "font-size: 14px; font-weight: bold; color: #ff00ff;"
            )  # Roxo para estudo
    ...
    def setup_ai_brain_tab(self):
        ...
        self.brain_status_text.setStyleSheet(
        ...
        self.functions_list.setStyleSheet(
            "color: #e0e0e0; background-color: #0b0f19; padding: 10px; border-radius: 5px;"
        )
    ...
    def setup_config_tab(self):
        ...
        self.model_selector.addItems(
            ["Qwen 1.5B (Local)", "Gemini Flash (Cloud)", "Llama 3 (Local)"]
        )
    ...
    def load_configurations(self):
        ...
        try:
            ...
            if config_path.exists():
                ...
                # Atualizar interface
                self.network_enabled.setCurrentText(
                    "Habilitado" if nm.get("enabled") else "Desabilitado"
                )
                self.google_drive_enabled.setCurrentText(
                    "Habilitado"
                    if nm.get("google_drive", {}).get("enabled")
                    else "Desabilitado"
                )
                self.local_network_enabled.setCurrentText(
                    "Habilitado"
                    if nm.get("local_network", {}).get("enabled")
                    else "Desabilitado"
                )
                self.encryption_enabled.setCurrentText(
                    "Habilitado"
                    if nm.get("privacy", {}).get("encrypt_packets")
                    else "Desabilitado"
                )
    ...
    def save_configurations(self):
        ...
        try:
            ...
            privacy["encrypt_packets"] = (
                self.encryption_enabled.currentText() == "Habilitado"
            )
            ...
            self.status_bar.showMessage(
                "✅ Configurações da Mente Coletiva salvas!", 3000
            )
    ...
    def setup_console_tab(self):
        ...
        self.log_area.setStyleSheet(
            "background-color: #000; color: #0f0; font-family: Consolas; padding: 10px;"
        )
    ...
    def _update_brain_status(self):
        ...
        try:
            ...
            brain_status = (
                "ONLINE - INTEGRADO"
                if is_brain_loaded
                else "OFFLINE - AGUARDANDO CONEXÃO"
            )
            ...
            hw_name = (
                hardware_manager.gpu_name if device != "cpu" else "AVX-512 Optimized"
            )
            ...
            if any("Mesh" in t for t in active_threads):
                stark_modules.append("• Network Mesh (ACTIVE)")
    ...
    def add_log_message(self, level, message):
        ...
        scroll_bar = (
            self.log_area.parent().parent().verticalScrollBar()
        )  # QLabel -> ScrollAreaChild -> ScrollArea -> ScrollBar
        if scroll_bar:
