...
class DatasetBuilder:
    ...
    def __init__(
        ...
        logger.info(
            f"DatasetBuilder initialized: {len(self.interactions)} interactions loaded"
        )
    ...
    def log_interaction(
        ...
        try:
            ...
            interaction.quality_score = self.quality_filter.calculate_quality_score(
                interaction
            )

            # Add to memory if quality check passes
            if not self.quality_filtering or self.quality_filter.should_include(
                interaction
            ):
                ...
                logger.debug(
                    f"Filtered out low-quality interaction (score: {interaction.quality_score:.2f})"
                )
    ...
    def _update_statistics(
        self, interaction: Interaction, loading: bool = False
    ) -> None:
    ...
    def to_instruct_format(
        ...
        for interaction in interactions:
            ...
            text = template.format(
                input=interaction.user_input, output=interaction.ai_response
            )
    ...
    def export_dataset(
        ...
        try:
            ...
            logger.info(
                f"Exported {len(data)} samples to {output_path} ({format} format)"
            )
            return len(data)
    ...
    def _filter_interactions(
        ...
        if date_range:
            ...
            filtered = [
                i for i in filtered if start_date <= i.metadata.timestamp <= end_date
            ]
    ...
    def clear_old_interactions(self, days: int = 30) -> int:
        ...
        try:
            ...
            self.interactions = [
                i for i in self.interactions if i.metadata.timestamp >= cutoff_date
            ]
