...
class ContinualLearner:
    ...
    def _ensure_dependencies(self):
        ...
        try:
            ...
            self.trainer = LocalTrainer(
                config=config, output_dir=self.data_dir / "models" / "continual"
            )
            return True
    ...
    def start(self):
        ...
        thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="ContinualLearner-Pulse"
        )
        thread.start()
        logger.info(
            "ðŸ§  JARVIS Learning Systems: Engaged (Threshold: %d)",
    ...
    def _monitoring_loop(self):
        ...
        while self.is_running:
            try:
                ...
                pending = (
                    self.feedback_db.count_pending()
                    if hasattr(self.feedback_db, "count_pending")
                    else 0
                )

                if pending >= self.feedback_threshold:
                    logger.info(
                        f"ðŸŽ¯ Significant feedback cluster detected: {pending} interactions. Initiating cycle..."
                    )
    ...
    def _execute_training_cycle(self):
        ...
        try:
            ...
            pairs = self.feedback_db.generate_preference_pairs(
                n_pairs=self.feedback_threshold
            )

            if len(pairs) < 10:
                logger.warning(
                    "âš ï¸ Insufficient valid preference pairs for quality DPO. Skipping cycle."
                )
                return
            ...
            train_file = (
                train_dir / f"dpo_cycle_{self.training_cycles}_{int(time.time())}.json"
            )
            ...
            logger.info(
                f"âœ… [TRAINING CYCLE COMPLETE] Cycle {self.training_cycles} successful. Duration: {duration:.2f}s"
            )
