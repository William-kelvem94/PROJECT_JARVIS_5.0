...
def create_shortcut(prefer_minimal: bool = False):
    ...
    Se `prefer_minimal` for True tenta usar `start_jarvis.bat` quando disponível.
    """
    ...
    if system == "Windows":
        ...
        minimal_script = (
            project_root / "scripts" / "launchers" / "start_jarvis.bat"
        )
        ...
        if prefer_minimal and minimal_script.exists():
            ...
            if minimal_script.exists() and default_script.exists():
                choice = (
                    input(
                        "Detectei 'start_jarvis.bat'. Criar atalho para (1) full (start_jarvis.bat) ou (2) minimal? [2]: "
                    )
                    or "2"
                )
                if choice.strip() == "1":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--minimal",
        action="store_true",
        help="Criar atalho apontando para start_jarvis.bat quando disponível",
    )
