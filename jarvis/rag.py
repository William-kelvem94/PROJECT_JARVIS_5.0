        ...
        with self._lock:
            if self.model is None:
                try:
                    ...
                    logger.info("RAG: Carregando modelo sentence-transformers (paraphrase-multilingual-MiniLM-L12-v2)...")
                    self.model = SentenceTransformer('paraphrase-multilingual-MiniLM-L12-v2')
                    logger.info("RAG: Modelo carregado com sucesso.")
                except ImportError:
                    logger.warning("sentence-transformers não instalado. RAG desativado.")
                    return False
                except Exception as e:
                    logger.error(f"Erro ao carregar modelo RAG: {e}")
                    return False
            return True
