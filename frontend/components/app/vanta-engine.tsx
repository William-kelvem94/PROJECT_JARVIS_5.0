    ...
    const draw = () => {
      ...
      for (const ring of RINGS) {
        ...
        for (let d = 0; d < ring.dotCount; d++) {
          ...
          const dx =
            cx +
            radiusX * Math.cos(da) * Math.cos(angle) -
            radiusY * Math.sin(da) * Math.sin(angle);
          const dy =
            cy +
            radiusX * Math.cos(da) * Math.sin(angle) +
      ...
      coronaGrad.addColorStop(0.45, `rgba(${r},${g},${b},${0.08 + v * 0.1})`);
      ...
      sphereGrad.addColorStop(
        1,
        `rgba(${Math.round(r * 0.25)},${Math.round(g * 0.25)},${Math.round(b * 0.25)},0.10)`
      );
      ctx.beginPath();
...
export const VantaController = ({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      vantaRef.current?.setVolume(0.05 + Math.random() * 0.05);
    }, 100);

    return () => clearInterval(interval);
  }, [vantaRef, isConnected]);
