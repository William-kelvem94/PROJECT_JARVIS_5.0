...
@pytest.mark.asyncio
async def test_add_memory_returns_false_for_duplicate(mock_memory):
    mem, mock_conn = mock_memory
    mock_conn.execute.return_value.fetchone.return_value = {"id": 1}
    result = await mem.add_memory("test_user", "same content", "fact", "conversation")
    assert result is False

@pytest.mark.asyncio
async def test_add_memory_returns_true_for_new(mock_memory):
    mem, mock_conn = mock_memory
    mock_conn.execute.return_value.fetchone.return_value = None
    result = await mem.add_memory("test_user", "new content", "fact", "conversation")
    assert result is True

@pytest.mark.asyncio
async def test_get_all_returns_memories(mock_memory):
    mem, mock_conn = mock_memory
