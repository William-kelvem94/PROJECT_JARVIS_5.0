...
class Capture(Base):
    ...
    ocr_results = relationship(
        "OCRResult", back_populates="capture", cascade="all, delete-orphan"
    )
    extracted_data = relationship(
        "ExtractedData", back_populates="capture", cascade="all, delete-orphan"
    )
...
class ExtractedData(Base):
    ...
    )  # personal, financial, business, documents
...
class DatabaseManager:
    ...
    def update_processing_status(
        self, capture_id: int, status: str, processed_at: datetime = None
    ):
    ...
    def search_extracted_data(
        self, query: str, data_type: str = None
    ) -> List[ExtractedData]:
        """Busca dados extraÃ­dos"""

        def _search(session):
            q = session.query(ExtractedData).filter(
                ExtractedData.field_value.contains(query)
            )
            if data_type:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        def _stats(session):
            ...
            data_types = (
                ...
                .group_by(ExtractedData.data_type)
                .all()
            )
            ...
            return {
                ...
                "processing_rate": (
                    processed_captures / total_captures if total_captures > 0 else 0
                ),
