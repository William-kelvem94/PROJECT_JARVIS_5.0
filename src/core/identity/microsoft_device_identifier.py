...
from src.utils.platform_compat import winreg, wmi
...
class MicrosoftDeviceIdentifier:
    ...
    def identify_complete_profile(self) -> Dict[str, Any]:
        ...
        try:
            ...
            profile = {
                "microsoft_account": (
                    asdict(self.microsoft_account) if self.microsoft_account else None
                ),
                "device_fingerprint": (
                    asdict(self.device_fingerprint) if self.device_fingerprint else None
                ),
                "google_drive_auth": (
                    asdict(self.google_drive_auth) if self.google_drive_auth else None
                ),
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["whoami", "/fqdn"], capture_output=True, text=True
                )
                        ...
                        while True:
                            try:
                                ...
                                if "@" in subkey_name:  # Parece email
                                    ...
                                    account_info.account_type = "microsoft"
                                    print(
                                        f"   âœ… Conta Microsoft via Identity: {subkey_name}"
                                    )
                                    break
                                i += 1
                            except OSError:
            ...
            print(
                f"   ðŸ“‹ Resultado: {account_info.account_type} - {account_info.account_email}"
            )
            ...
            try:
                ...
                for board in wmi_conn.Win32_BaseBoard():
                    if board.SerialNumber and board.SerialNumber.strip():
                        fingerprint_data["motherboard_serial"] = (
                            board.SerialNumber.strip()
                        )
                    break
                ...
                fingerprint_data["motherboard_serial"] = str(
                    uuid.getnode()
                )  # MAC como fallback
    ...
    def _detect_google_drive(self) -> GoogleDriveAuth:
        ...
        drive_auth = GoogleDriveAuth(
            is_authenticated=False, account_email="unknown", sync_enabled=False
        )
        ...
        try:
            ...
            if drive_auth.is_authenticated:
                ...
                print("   âŒ Google Drive nÃ£o autenticado")
    ...
    def get_google_drive_jarvis_folder(self) -> Optional[str]:
        ...
        try:
            ...
            if self.google_drive_auth and self.google_drive_auth.drive_path:
                jarvis_folder = (
                    Path(self.google_drive_auth.drive_path) / f"JARVIS_{username}"
                )
    ...
    def load_cached_identity(self) -> bool:
        ...
        try:
            ...
            last_update = datetime.fromisoformat(
                cache_data.get("last_update", "2000-01-01")
            )
    ...
    def get_identity_summary(self) -> str:
        ...
        auth_status = (
            "âœ… AUTORIZADO" if self.is_authorized_device() else "âŒ NÃƒO AUTORIZADO"
        )
        google_status = (
            "âœ… CONECTADO"
            if (self.google_drive_auth and self.google_drive_auth.is_authenticated)
            else "âŒ DESCONECTADO"
        )
