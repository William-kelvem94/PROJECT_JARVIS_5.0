...
class MultimodalFusion:
    ...
    def __init__(self, embedding_model="paraphrase-multilingual-MiniLM-L12-v2", embedding_dim=384):
        """
        Args:
    ...
    def fuse_contexts(
        ...
        if audio_emotion:
            ...
            active_weights = [w * (0.5 + 0.5 * audio_weight_mod) for w in active_weights]
        ...
        logger.debug(f"Fused {len(embeddings)} modalities with weights {normalized_weights}")
    ...
    def adaptive_fusion(
        ...
        return self.fuse_contexts(text, image_description, audio_emotion, custom_weights=weights)
