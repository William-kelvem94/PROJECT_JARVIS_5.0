...
class AIConfig(BaseModel):
    ...
    def check_api_keys(self) -> 'AIConfig':
        ...
        if self.provider == 'gemini' and not self.gemini_api_key:
            logger.warning("Gemini API key not configured explicitly (using environment fallback?)")
        return self
...
class SystemManifest(BaseSettings):
    ...
    def validate_hardware_requirements(self) -> Dict[str, bool]:
        ...
        if self.security.require_hardware_acceleration and not self.security.allow_cpu_fallback:
