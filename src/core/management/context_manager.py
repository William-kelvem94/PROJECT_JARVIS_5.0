...
class ContextManager:
    ...
    def _init(self):
        ...
        logger.info(
            f"✅ ContextManager inicializado. Usuário Ativo: {self.current_user}"
        )
    ...
    def switch_user(self, name: str):
        ...
        if user:
            ...
            logger.info(
                f"🔄 Workspace alternado para: {self.current_user} ({self.current_role})"
            )
            return True
        return False
    ...
    def record_interaction(self, sentiment: str, was_helpful: bool):
        ...
        if sentiment == "positive":
            self.user_preferences["proactivity"] = min(
                1.0, self.user_preferences["proactivity"] + 0.05
            )

        logger.debug(
            f"📈 Bond Level: {self.bond_level} | Proactivity: {self.user_preferences['proactivity']:.2f}"
        )
