...
class ModuleHealth:
    ...
    def update_metrics(
        self, memory_mb: float = 0.0, cpu_percent: float = 0.0, response_ms: float = 0.0
    ):
...
class UniversalRecoveryManager:
    ...
    def trigger_emergency_mode(self, active: bool = True):
        ...
        try:
            ...
            emit_log_sync(
                f"SISTEMA: Protocolo de SobrevivÃªncia {state}", level="CRITICAL"
            )
    ...
    def _register_default_strategies(self):
        ...
        self.register_strategy(
            FailureType.NETWORK_FAILURE, self._recover_network_failure
        )
        ...
        self.register_strategy(
            FailureType.FILE_CORRUPTION, self._recover_file_corruption
        )

        # Permission Error Recovery
        self.register_strategy(
            FailureType.PERMISSION_ERROR, self._recover_permission_error
        )

        logger.info(
            f"ðŸ“‹ Registered {len(self.recovery_strategies)} recovery strategy types"
        )
    ...
    def _trigger_recovery(
        ...
        recovery_thread = threading.Thread(
            target=self._execute_recovery_async, args=(failure_event,), daemon=True
        )
    ...
    async def _execute_recovery(self, failure_event: FailureEvent):
        ...
        logger.warning(
            f"ðŸ”§ Starting auto-recovery for {failure_event.failure_type.value} in {failure_event.module_name}"
        )
        ...
        try:
            ...
            if not strategies:
                logger.error(
                    f"âŒ No recovery strategies available for {failure_event.failure_type.value}"
                )
                return
            ...
            for strategy in strategies:
                strategy_name = (
                    f"{failure_event.failure_type.value}::{strategy.__name__}"
                )
            ...
            for strategy_func, success_rate in strategies_with_rates:
                if len(failure_event.recovery_attempts) >= self.max_recovery_attempts:
                    logger.error(
                        f"âŒ Max recovery attempts reached for {failure_event.module_name}"
                    )
                    break

                logger.info(
                    f"ðŸ”§ Trying recovery strategy: {strategy_func.__name__} (success rate: {success_rate:.1%})"
                )
                ...
                try:
                    ...
                    result = await self._execute_strategy_safely(
                        strategy_func, failure_event
                    )
                    ...
                    if result.get("success"):
                        logger.info(
                            f"âœ… Recovery successful: {strategy_func.__name__}"
                        )

                        # Update module health
                        if failure_event.module_name in self.module_health:
                            self.module_health[
                                failure_event.module_name
                            ].record_recovery()

                        # Update strategy success rate
                        self._update_strategy_success_rate(
                            strategy_func.__name__, failure_event.failure_type, True
                        )
                        break
                    else:
                        logger.warning(
                            f"âš ï¸ Recovery strategy failed: {strategy_func.__name__}"
                        )
                        self._update_strategy_success_rate(
                            strategy_func.__name__, failure_event.failure_type, False
                        )

                except Exception as e:
                    execution_time = (time.time() - start_time) * 1000
                    logger.error(
                        f"âŒ Recovery strategy error: {strategy_func.__name__}: {e}"
                    )
            ...
            if not any(
                ...
                for attempt in failure_event.recovery_attempts
            ):
                logger.warning(
                    "âš ï¸ All recovery strategies failed, escalating to fallback system"
                )
    ...
    async def _execute_strategy_safely(
        self, strategy_func: Callable, failure_event: FailureEvent
    ) -> Dict:
        ...
        try:
            ...
            return {
                "success": False,
                "action": "Strategy timed out",
                "side_effects": ["Timeout"],
            }
        except Exception as e:
            return {
                "success": False,
                "action": f"Strategy error: {e}",
                "side_effects": [str(e)],
            }

    def _update_strategy_success_rate(
        self, strategy_name: str, failure_type: FailureType, success: bool
    ):
        ...
        self.strategy_success_rates[strategy_key] = max(
            0.01, min(0.99, new_rate)
        )  # Clamp to [0.01, 0.99]
    ...
    def _recover_via_pip_install(self, failure_event: FailureEvent) -> Dict:
        ...
        try:
            ...
            if not package_name:
                return {
                    "success": False,
                    "action": "Could not identify missing package",
                    "side_effects": [],
                }
        ...
        try:
            ...
            if history_file.exists():
                with open(history_file, "r", encoding="utf-8") as f:
                    ...
                    logger.info(
                        f"ðŸ“Š Loaded {len(self.strategy_success_rates)} strategy success rates"
                    )
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        successful_recoveries = len(
            [r for r in self.recovery_history if r.status == RecoveryStatus.SUCCESS]
        )
        ...
        avg_recovery_time = (
            sum(r.execution_time_ms for r in self.recovery_history) / total_recoveries
        )
        ...
        return {
            ...
            "healthy_modules": len(
                [h for h in self.module_health.values() if h.is_healthy]
            ),
        }
...
universal_recovery_manager: Optional[UniversalRecoveryManager] = None


def get_universal_recovery_manager() -> UniversalRecoveryManager:
    """Get global universal recovery manager instance"""
    global universal_recovery_manager
    if universal_recovery_manager is None:
        universal_recovery_manager = UniversalRecoveryManager()
    return universal_recovery_manager


def register_module_for_monitoring(module_name: str) -> ModuleHealth:
    """Convenience function to register module for monitoring"""
    return get_universal_recovery_manager().register_module(module_name)


def trigger_recovery_for_exception(
    module_name: str, exception: Exception, severity: int = 5
):
    ...
    recovery_system = get_universal_recovery_manager()
