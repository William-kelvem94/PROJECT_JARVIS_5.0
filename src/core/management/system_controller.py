...
class SystemController:
    ...
    def read_file_content(self, file_path: str) -> Optional[str]:
        ...
        try:
            if not SecurityManager.validate_path_access(file_path):
                logger.warning(f"ðŸ›¡ï¸ Bloqueio Anti-Genesis: Tentativa de leitura em {file_path}")
                return None
    ...
    def safe_code_update(self, file_path: str, new_code: str) -> Dict[str, Any]:
        ...
        if not SecurityManager.validate_path_access(file_path):
             return {"status": "error", "message": "Acesso negado: O SecurityManager bloqueou a modificaÃ§Ã£o deste recurso crÃ­tico."}
        ...
        try:
            ...
            try:
                ...
                return {"status": "error", "message": f"Erro de sintaxe no cÃ³digo gerado: {se}"}
...
"""
            ...
            if "SUCCESS" not in result.stdout:
                logger.error(f"âŒ Falha no teste de importaÃ§Ã£o: {result.stdout} {result.stderr}")
                return {"status": "error", "message": f"O cÃ³digo falhou no teste de carregamento: {result.stdout}"}
