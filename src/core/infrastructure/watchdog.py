...
class WatchdogSystem:
    ...
    def register_component(
        self, name: str, heartbeat_interval: float = 5.0, pid: Optional[int] = None
    ):
        ...
        with self._lock:
            ...
            logger.info(
                f"🐶 Component registered: {name} (Interval: {heartbeat_interval}s)"
            )
    ...
    def start(self):
        ...
        self._thread = threading.Thread(
            target=self._monitor_loop, name="WatchdogCore", daemon=True
        )
        self._thread.start()
            ...
            try:
                with self._lock:
                    ...
                    for name, comp in self._components.items():
                        ...
                        dead_threshold = comp.heartbeat_interval * (
                            10 if is_booting else 3
                        )
                        slow_threshold = comp.heartbeat_interval * (
                            5 if is_booting else 1.5
                        )
                        ...
                        if delta > dead_threshold:
                            ...
                            if comp.status != ComponentStatus.DEAD:
                                logger.error(
                                    f"💀 Watchdog: {name} is DEAD (No heartbeat for {delta:.1f}s)"
                                )
                            ...
                            if comp.status != ComponentStatus.UNHEALTHY:
                                logger.warning(
                                    f"⚠️ Watchdog: {name} is SLOW (Delayed {delta:.1f}s)"
                                )
    ...
    def _handle_failure(self, comp: ComponentHealth):
        ...
        comp.last_heartbeat = (
            datetime.now()
        )  # Fake heartbeat to prevent instant re-trigger
    ...
    def _check_system_health(self):
        ...
        try:
            ...
            if mem.percent > 90:
                logger.error(
                    f"⚠️ System Memory Critical: {mem.percent}% - Triggering EMERGENCY cleanup"
                )
            ...
            # MEMORY WARNING GUARD (>75%)
                ...
                logger.warning(
                    f"⚠️ System Memory High: {mem.percent}% - Triggering standard cleanup"
                )
