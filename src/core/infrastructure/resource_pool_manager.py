...
class ResourcePool(Generic[T]):
    ...
    @asynccontextmanager
    async def acquire(
        self, user_id: Optional[str] = None, timeout: Optional[float] = None
    ):
    ...
    async def _acquire_resource(
        self, user_id: Optional[str], timeout: float
    ) -> PooledResource[T]:
        ...
        while time.time() - start_time < timeout:
            ...
            if self._is_circuit_breaker_open():
                raise RuntimeError(
                    f"Circuit breaker open for pool '{self.config.name}'"
                )
            ...
            with self._lock:
                ...
                if self._available_queue:
                    ...
                    if resource and resource.state == ResourceState.AVAILABLE:
                        ...
                        if resource.acquire(user_id):
                            ...
                            logger.debug(
                                f"🔓 Acquired resource {resource_id[:8]} from pool '{self.config.name}'"
                            )
                            return resource
                ...
                if len(self._resources) < self.config.max_size:
                    try:
                        ...
                        logger.error(
                            f"Failed to create resource in pool '{self.config.name}': {e}"
                        )
    ...
    async def _release_resource(self, resource: PooledResource[T]):
        ...
        with self._lock:
            ...
            if resource.state == ResourceState.AVAILABLE:
                ...
                logger.debug(
                    f"🔐 Released resource {resource.resource_id[:8]} to pool '{self.config.name}' (used {duration:.2f}s)"
                )
        ...
        try:
            ...
            if self.config.resource_ttl_seconds:
                expires_at = datetime.now() + timedelta(
                    seconds=self.config.resource_ttl_seconds
                )
            ...
            logger.debug(
                f"➕ Created resource {resource.resource_id[:8]} in pool '{self.config.name}'"
            )
            ...
            logger.error(
                f"Failed to create resource for pool '{self.config.name}': {e}"
            )
            self._record_circuit_breaker_failure()
            raise
    ...
    async def _destroy_resource(self, resource: PooledResource[T]):
        ...
        try:
            ...
            logger.debug(
                f"❌ Destroyed resource {resource.resource_id[:8]} from pool '{self.config.name}'"
            )
    ...
    async def _check_resource_health(
        ...
        try:
            ...
            if is_healthy:
                ...
                if (
                    resource.metrics.health_check_failures
                    >= self.config.max_health_check_failures
                ):
                    logger.warning(
                        f"Resource {resource.resource_id[:8]} exceeded health check failures"
                    )
                    await self._destroy_resource(resource)
                    return False
            ...
            logger.error(
                f"Health check error for resource {resource.resource_id[:8]}: {e}"
            )
            resource.mark_unhealthy()
            return False
    ...
    def _health_check_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.error(
                    f"Health check loop error for pool '{self.config.name}': {e}"
                )
                time.sleep(5.0)
    ...
    async def _run_health_checks(self):
        ...
        with self._lock:
            idle_resources = [
                ...
                if resource.state == ResourceState.AVAILABLE
                and resource.metrics.idle_time
    ...
    async def _check_scaling(self):
        ...
        with self._lock:
            ...
            if (
                ...
                and total_resources < self.config.max_size
            ):
                ...
                logger.info(
                    f"📈 Scaling up pool '{self.config.name}' by {scale_up_count} resources (utilization: {utilization:.2%})"
                )
                ...
                for _ in range(scale_up_count):
                    try:
                        ...
                        logger.error(
                            f"Failed to scale up pool '{self.config.name}': {e}"
                        )
                        break
            ...
            # Scale down
                ...
                and total_resources > self.config.min_size
            ):
    ...
    async def _cleanup_expired_resources(self):
        ...
        with self._lock:
            ...
            for resource in self._resources.values():
                ...
                if (
                    ...
                    and resource.metrics.idle_time > self.config.max_idle_seconds
                ):
        ...
        for resource in resources_to_cleanup:
            logger.debug(
                f"🧹 Cleaning up resource {resource.resource_id[:8]} from pool '{self.config.name}'"
            )
            await self._destroy_resource(resource)
    ...
    def _is_circuit_breaker_open(self) -> bool:
        ...
        if self._circuit_breaker_last_failure:
            ...
            if elapsed > self.config.circuit_breaker_recovery_timeout:
                logger.info(
                    f"🔄 Circuit breaker recovering for pool '{self.config.name}'"
                )
                self._circuit_breaker_open = False
                self._circuit_breaker_failures = 0
                return False
    ...
    def _record_circuit_breaker_failure(self):
        ...
        if (
            ...
            if not self._circuit_breaker_open:
                logger.error(
                    f"💥 Circuit breaker opened for pool '{self.config.name}' after {self._circuit_breaker_failures} failures"
                )
