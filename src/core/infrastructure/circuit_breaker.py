...
class CircuitBreaker:
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            call_result = CallResult(
                success=True, duration_seconds=duration, response_data=result
            )
    ...
    def _transition_to_open(self):
        ...
        if self.state != CircuitState.OPEN:
            logger.warning(
                f"🔴 Circuit breaker '{self.config.name}' OPENED (failures: {self._consecutive_failures})"
            )
    ...
    def _transition_to_half_open(self):
        ...
        logger.info(
            f"🟡 Circuit breaker '{self.config.name}' HALF-OPEN (attempting recovery)"
        )
    ...
    def _transition_to_closed(self):
        ...
        logger.info(
            f"🟢 Circuit breaker '{self.config.name}' CLOSED (recovery successful)"
        )
    ...
    def _execute_fallback(self, *args, **kwargs) -> Any:
        ...
        raise CircuitBreakerOpenError(
            f"Circuit breaker '{self.config.name}' is open and no fallback available"
        )
                ...
                if self._health_check_function:
                    ...
                    try:
                        ...
                        result = CallResult(
                            ...
                            duration_seconds=duration,
                            failure_type=(
                                None if is_healthy else FailureType.SERVICE_UNAVAILABLE
                            ),
                        ...
                        logger.error(
                            f"Health check error for '{self.config.name}': {e}"
                        )
        ...
        with self._lock:
            return {
                ...
                "metrics": {
                    ...
                    "fastest_response": (
                        self.metrics.fastest_response
                        if self.metrics.fastest_response != float("inf")
                        else 0
                    ),
...
class CircuitBreakerManager:
    ...
    def start(self):
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, name="circuit_breaker_monitor", daemon=True
        )
                ...
                with self._lock:
                    for name, circuit_breaker in self._circuit_breakers.items():
                        ...
                        if stats["state"] == "open":
                            ...
                            logger.warning(
                                f"🚨 Circuit breaker '{name}' is OPEN (failure rate: {failure_rate:.1f}%)"
                            )
                        ...
                        # Alert on high failure rates
                            ...
                            logger.warning(
                                f"⚠️ Circuit breaker '{name}' high failure rate: {recent_rate:.1f}%"
                            )
...
if __name__ == "__main__":
    ...
    async def test_circuit_breaker():
        ...
        print(
            f"Success rate: {(1 - final_stats['metrics']['failure_rate']) * 100:.1f}%"
        )
        ...
        for name, stats in all_stats["circuit_breakers"].items():
            ...
            print(
                f"  Success rate: {(1 - stats['metrics']['failure_rate']) * 100:.1f}%"
            )
