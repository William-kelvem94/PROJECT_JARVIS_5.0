    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "utilization_percent": (self._total_allocated / self.max_size_bytes) * 100,
...
class SessionData:
    ...
    def set_state(self, new_state: SessionState):
        ...
        with self._lock:
            ...
            logger.debug(f"📊 Session {self.session_id[:8]}: {old_state.value} -> {new_state.value}")
    ...
    def get_thread_pool(self, pool_type: ThreadPoolType, 
        ...
        with self._lock:
            if pool_type not in self._thread_pools:
                ...
                default_configs = {
                    ...
                    ThreadPoolType.CPU_BOUND: ThreadPoolConfig(pool_type, 1, multiprocessing.cpu_count()),
                ...
                logger.debug(f"🧵 Created {pool_type.value} thread pool for session {self.session_id[:8]}")
            
            return self._thread_pools[pool_type]
    
    def submit_task(self, pool_type: ThreadPoolType, 
        ...
        with self._lock:
            ...
            self.metrics.peak_threads = max(self.metrics.peak_threads, self.metrics.active_threads)
            self.metrics.update_activity()
    ...
    def _on_future_done(self, future: Future):
        ...
        with self._lock:
            ...
            if future.exception():
                ...
                logger.warning(f"Task error in session {self.session_id[:8]}: {future.exception()}")
    ...
    def wait_for_tasks(self, timeout: Optional[float] = None) -> bool:
        ...
        while self._active_futures:
            if timeout and (time.time() - start_time) > timeout:
                logger.warning(f"Timeout waiting for tasks in session {self.session_id[:8]}")
                return False
    ...
    def allocate_shared_memory(self, key: str, size_bytes: int, 
                             memory_manager: SharedMemoryManager) -> Optional[mmap.mmap]:
    ...
    def get_shared_memory(self, key: str, 
                         memory_manager: SharedMemoryManager) -> Optional[mmap.mmap]:
        ...
        with self._lock:
            ...
            for pool_type, pool in self._thread_pools.items():
                try:
                    ...
                    logger.debug(f"Shutdown {pool_type.value} pool for session {self.session_id[:8]}")
                except Exception as e:
...
class MultithreadSessionManager:
    ...
    def get_session(self, session_id: str) -> Optional[SessionData]:
        ...
        with self._lock:
            ...
            if session and session.state not in [SessionState.TERMINATED, SessionState.TERMINATING]:
    ...
    def submit_task_to_session(self, session_id: str, 
                              pool_type: ThreadPoolType,
                              func: Callable, *args, **kwargs) -> Optional[Future]:
    ...
    def _cleanup_expired_sessions(self):
        ...
        with self._lock:
            for session_id, session in self._sessions.items():
                ...
                if (session.metrics.idle_time > self.session_timeout_seconds and 
                    session.state in [SessionState.IDLE, SessionState.ACTIVE]):
...
if __name__ == "__main__":
    ...
    def test_session_manager():
        ...
        if session:
            ...
            # Test shared memory
            mm = session.allocate_shared_memory("test_data", 1024, session_manager.shared_memory)
            if mm:
