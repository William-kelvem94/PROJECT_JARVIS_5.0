...
class PriorityScheduler:
    ...
    def schedule_task(self, 
        ...
        logger.debug(f"📋 Scheduled task '{name}' (ID: {task_id[:8]}, Priority: {priority.name})")
        ...
        for priority in TaskPriority:
            ...
            for _ in range(len(queue)):
                ...
                if task.task_type == TaskType.PERIODIC and task.interval_seconds:
                    task.next_execution = current_time + timedelta(seconds=task.interval_seconds)
    ...
    async def _execute_task(self, task: SchedulerTask):
        ...
        try:
            ...
            asyncio.create_task(self._handle_task_completion(task, asyncio_task, task_start))
            
        except Exception as e:
            await self._handle_task_error(task, e, task_start)
    
    async def _handle_task_completion(self, task: SchedulerTask, asyncio_task: asyncio.Task, start_time: float):
        ...
        try:
            ...
            task.metrics.average_runtime = task.metrics.total_runtime / task.metrics.execution_count
    ...
    async def _handle_task_error(self, task: SchedulerTask, error: Exception, start_time: float):
        ...
        if task.metrics.failure_count < task.max_retries:
            ...
            task.next_execution = datetime.now() + timedelta(seconds=2 ** task.metrics.failure_count)
            logger.debug(f"🔄 Retrying task '{task.name}' (attempt {task.metrics.failure_count + 1})")
        else:
        ...
        while self._running:
            try:
                ...
                if self.system_load.overall_load > 0.8:
                    logger.warning(f"🔥 High system load: CPU={self.system_load.cpu_percent:.1f}% RAM={self.system_load.memory_percent:.1f}%")
...
if __name__ == "__main__":
    ...
    async def test_scheduler():
        ...
        # Schedule tasks
        priority_scheduler.schedule_task("high_task", high_priority_task, TaskPriority.HIGH)
        priority_scheduler.schedule_task("normal_task", normal_task, TaskPriority.NORMAL)
        priority_scheduler.schedule_task("low_task", low_priority_task, TaskPriority.LOW)
        
        # Schedule periodic task
        async def periodic_task():
            print("🔄 Periodic task executed")
        
        priority_scheduler.schedule_periodic("periodic_test", periodic_task, 3.0, TaskPriority.NORMAL)
