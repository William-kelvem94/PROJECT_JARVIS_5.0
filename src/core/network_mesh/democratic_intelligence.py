...
import hashlib
import time
from src.utils.platform_compat import winreg
import psutil
import subprocess
...
class DemocraticNetworkIntelligence:
    ...
    def __init__(self, jarvis_core_path: str, target_account: Optional[str] = None):
        ...
        if not self.is_authorized:
            print(
                f"âš ï¸ Dispositivo nÃ£o autorizado. Conta detectada: {self.device_account}"
            )
        ...
        print("ðŸ›ï¸ Democratic Network Intelligence inicializado")
        print(f"   ðŸ“± Dispositivo: {platform.node()}")
        print(f"   ðŸ‘¤ Conta autorizada: {self.device_account}")
        print(
            f"   ðŸ’ª Specs: {self.local_specs.cpu_cores}C/{self.local_specs.total_ram_gb:.0f}GB"
        )
...
>>>>>>> dev-new-version
    ...
    def _verify_account_authorization(self) -> bool:
        ...
        return (
            self.device_account == self.target_account
            or self.device_account.split("@")[0] == self.target_account.split("@")[0]
        )
    ...
    def _setup_consolidation_folder(self) -> Optional[Path]:
        ...
        consolidation_folder = (
            Path(self.gdrive_base_path) / f"JARVIS_Democratic_{account_name}"
        )
    ...
    async def request_distributed_task(self, task_type: TaskType, **kwargs) -> str:
        ...
        task = DistributedTask(
            task_id=hashlib.sha256(
                f"{time.time()}:{task_type.value}".encode()
            ).hexdigest()[:16],
            task_type=task_type,
    ...
    def _get_required_capabilities(self, task_type: TaskType) -> List[DeviceCapability]:
        ...
        capability_map = {
            TaskType.VOICE_TRAINING: [
                DeviceCapability.CUDA_TRAINING,
                DeviceCapability.AUDIO_PROCESSING,
            ],
            TaskType.FACE_TRAINING: [
                DeviceCapability.CUDA_TRAINING,
                DeviceCapability.VISION_PROCESSING,
            ],
            TaskType.LLM_FINETUNING: [
                DeviceCapability.CUDA_TRAINING,
                DeviceCapability.MEMORY_INTENSIVE,
            ],
            TaskType.MEMORY_CONSOLIDATION: [
                DeviceCapability.MEMORY_INTENSIVE,
                DeviceCapability.STORAGE_BACKUP,
            ],
            TaskType.SYSTEM_BACKUP: [
                DeviceCapability.STORAGE_BACKUP,
                DeviceCapability.NETWORK_BRIDGE,
            ],
            TaskType.HEAVY_INFERENCE: [
                DeviceCapability.CUDA_TRAINING,
                DeviceCapability.COMPUTE_INTENSIVE,
            ],
    ...
    def _generate_local_bid(self, task: DistributedTask) -> ElectionBid:
        ...
        capability_score = self._calculate_capability_score(
            task.required_capabilities, current_specs
        )
    ...
    def _calculate_capability_score(
        self, required_caps: List[DeviceCapability], specs: DeviceSpecs
    ) -> float:
    ...
    async def consolidate_distributed_training(
        self, task_id: str, model_parts: List[str]
    ):
    ...
    def get_network_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "election_status": (
                self.election_status.value if self.election_status else None
            ),
...
async def democratic_training_demo():
    ...
    print(
        f"   ðŸ’ª CPU: {status['specs']['cpu_cores']} cores @ {status['specs']['cpu_freq_mhz']:.0f}MHz"
    )
    print(
        f"   ðŸ§  RAM: {status['specs']['total_ram_gb']:.1f}GB (disponÃ­vel: {status['specs']['available_ram_gb']:.1f}GB)"
    )
...
>>>>>>> dev-new-version
    ...
    print(
        f"   ðŸ‘‘ Papel atual: {final_status['current_role'] or 'Aguardando eleiÃ§Ã£o'}"
    )
