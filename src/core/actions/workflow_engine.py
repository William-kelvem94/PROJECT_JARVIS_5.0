...
class WorkflowEngine:
    ...
    def _register_default_handlers(self):
        ...
        self.action_handlers = {
            ...
            "hotkey": lambda params: advanced_action_controller.hotkey(
                *params.get("keys", [])
            ),
            "wait": lambda params: time.sleep(params.get("duration", 1)),
            "open_app": lambda params: advanced_action_controller.open_application(
                params.get("app_name")
            ),
            "close_app": lambda params: advanced_action_controller.close_application(
                params.get("app_name")
            ),
        }
    ...
    def execute_workflow(
        self, workflow_name: str, on_progress: Callable[[str], None] = None
    ) -> bool:
        """Executa um workflow"""
        if workflow_name not in self.workflows:
        ...
        logger.info(
            f"ðŸš€ Executando workflow: {workflow_name} ({len(workflow.steps)} passos)"
        )
        ...
        try:
            for i, step in enumerate(workflow.steps, 1):
                logger.info(
                    f"Passo {i}/{len(workflow.steps)}: {step.type} - {step.description}"
                )

                if on_progress:
                    on_progress(
                        f"Passo {i}/{len(workflow.steps)}: {step.description or step.type}"
                    )
...
def create_example_workflows():
    ...
    workflow_engine.add_step(
        "pesquisar_google",
        WorkflowStep(
            type="wait", params={"duration": 2}, description="Aguardar carregamento"
        ),
    )
    ...
    workflow_engine.add_step(
        "pesquisar_google",
        WorkflowStep(
            type="key", params={"key": "enter"}, description="Executar pesquisa"
        ),
    )
