...
class SystemIntegrator:
    ...
    def __init__(self, audit_log_path: Optional[Path] = None):
        ...
        self.protected_processes = {
            'system', 'wininit.exe', 'csrss.exe', 'services.exe',
    ...
    def _audit_log(self, operation: SystemOperation, details: str, success: bool = True):
            ...
            for session in sessions:
                if session.Process:
                    ...
                    if app_name.lower() in proc_name:
                        ...
                        logger.info(f"âœ… Set volume for {proc_name}: {volume*100:.0f}%")
                        return True
    ...
    def get_process_info(self, process_name: Optional[str] = None) -> List[ProcessInfo]:
        ...
        try:
            for proc in psutil.process_iter(['name', 'pid', 'exe', 'cpu_percent', 'memory_info', 'status']):
                try:
                    info = proc.info
    ...
    def minimize_window(self, title_substring: str) -> bool:
        ...
        try:
            ...
            self._audit_log(
                SystemOperation.MINIMIZE_WINDOW,
                f"{window.title}"
            )
...
if __name__ == "__main__":
    ...
    for proc in processes:
        print(f"   {proc.name} (PID: {proc.pid}) - CPU: {proc.cpu_percent}% - RAM: {proc.memory_mb:.1f}MB")
