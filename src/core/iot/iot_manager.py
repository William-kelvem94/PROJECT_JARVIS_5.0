...
class IOTManager:
    ...
    def __init__(self):
        self.ha_url = config.get_setting(
    ...
    def control_device(
        ...
        if not self.is_configured:
            logger.warning(
                "âš ï¸ Home Assistant nÃ£o configurado. Adicione 'iot.ha_token' ao ai_config.yaml"
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"ðŸ  Enviando comando IoT: {domain}.{command} para {device_id}"
            )
