...
class YOLOModelResource:
    ...
    def load(self):
        ...
        try:
            ...
            logger.info(
                f"🎯 Loaded YOLO model {Path(self.model_path).name} on {self.device} ({self.load_time:.2f}s)"
            )
            return True
...
class YOLOModelFactory(ResourceFactory[YOLOModelResource]):
    ...
    def create_resource(self) -> YOLOModelResource:
        ...
        if resource.load():
            ...
            raise RuntimeError("Failed to create YOLO model resource")
...
class OptimizedYOLOPipeline:
    ...
    def __init__(self, models_dir: Path, device: str = "cpu"):
        ...
        self._pending_batches: Dict[DetectionQuality, List[DetectionRequest]] = (
            defaultdict(list)
        )
        ...
        try:
            ...
            if self.enable_process_workers:
                ...
                process_worker_factory.configure_worker_type(
                    WorkerType.VISION_ANALYZER, yolo_worker_config
                )
...
>>>>>>> dev-new-version
        try:
            ...
            self.avg_inference_time = self.total_inference_time / max(
    ...
    async def detect_batch(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"Batch detection failed for request {requests[i].request_id}: {result}"
                )
            else:
            ...
            try:
                ...
                for quality, pending_requests in self._pending_batches.items():
                    if len(pending_requests) >= self._batch_size:
                        ...
                        self._pending_batches[quality] = pending_requests[
                            self._batch_size :
                        ]
                        ...
                        age_ms = (
                            datetime.now() - oldest_request.timestamp
                        ).total_seconds() * 1000
    ...
    async def _process_single_request(self, request: DetectionRequest):
        ...
        try:
            if self.enable_process_workers:
                ...
                worker_result = process_worker_factory.get_task_result(
                    task_id, timeout=30.0
                )
                ...
                if worker_result and worker_result.get("success"):
                    ...
                    result = DetectionResult(
                        ...
                        inference_time_ms=worker_result.get("duration_seconds", 0)
                        * 1000,
            ...
            error_result = DetectionResult(
                ...
                image_shape=(
                    (
                        int(request.image.shape[0]),
                        int(request.image.shape[1]),
                        int(request.image.shape[2]),
                    )
    ...
    async def _process_request_batch(self, batch: List[DetectionRequest]):
        ...
        try:
            ...
            logger.debug(
                f"Processed batch of {len(batch)} requests in {batch_time:.1f}ms"
            )
    ...
    async def _run_batch_inference(
        self, requests: List[DetectionRequest], model_resource: YOLOModelResource
    ) -> List[DetectionResult]:
        ...
        try:
            ...
            for i, (request, result) in enumerate(zip(requests, results)):
                ...
                if request.class_filter:
                    detections = [
                        d for d in detections if d.class_name in request.class_filter
                    ]
                ...
                result_obj = DetectionResult(
                    ...
                    / len(requests),  # Divide by batch size
                    total_time_ms=inference_time / len(requests),
                    image_shape=request.image.shape,
                    model_version="yolov8_batch",
                )
                ...
                async with self._model_pool.acquire() as model_resource:
                    ...
                    if request.class_filter:
                        detections = [
                            d
                            for d in detections
                            if d.class_name in request.class_filter
                        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "queues": {"request_queues": queue_sizes, "pending_batches": batch_sizes},
...
async def initialize_yolo_pipeline(
    models_dir: Path, device: str = "cpu"
