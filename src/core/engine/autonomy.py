...
class ConfidenceAssessor:
    ...
    def assess_confidence(
        ...
        if context.confidence_history:
            recent_avg = sum(context.confidence_history[-10:]) / len(
                context.confidence_history[-10:]
            )
    ...
    def update_from_outcome(self, decision: Decision, success: bool) -> None:
        ...
        recent_successes = sum(
            1 for h in list(self.history)[-20:] if h.get("success", False)
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        avg_confidence = sum(h["confidence"] for h in recent_history) / len(
            recent_history
        )
...
class DecisionEngine:
    ...
    def make_decision(self, context: DecisionContext) -> Decision:
        ...
        strategy = self.mode_strategies.get(self.mode, self._passive_strategy)
    ...
    def _active_strategy(self, context: DecisionContext) -> Decision:
        ...
        if context.user_input:
            ...
            confidence = self.confidence_assessor.assess_confidence(context, "respond")
            ...
            confidence = self.confidence_assessor.assess_confidence(context, "suggest")
    ...
    def _learning_strategy(self, context: DecisionContext) -> Decision:
        ...
        confidence = self.confidence_assessor.assess_confidence(context, "learn")
    ...
    def _exploration_strategy(self, context: DecisionContext) -> Decision:
        ...
        confidence = self.confidence_assessor.assess_confidence(
            context, "explore", factors={"exploration_bonus": 0.8}
        )
...
class MetaLearningController:
    ...
    def analyze_performance_trends(self, window_size: int = 100) -> Dict[str, float]:
        """
    ...
    def record_performance(
        self, task: str, performance: float, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """
...
class AutonomyCore:
    ...
    def __init__(
        ...
        self.decision_engine = DecisionEngine(
            mode=initial_mode, confidence_threshold=confidence_threshold
        )
    ...
    def start(self) -> None:
        ...
        self.background_thread = threading.Thread(
            target=self._background_loop, daemon=True, name="AutonomyCoreBackground"
        )
            ...
            if execute_callback and self._should_execute(decision):
                try:
                    ...
                    self.meta_learning_controller.record_performance(
                        task=decision.action, performance=1.0 if success else 0.0
                    )
    ...
    def change_mode(self, new_mode: AutonomyMode, reason: str = "") -> None:
        """
...
if __name__ == "__main__":
    ...
    autonomy.meta_learning_controller.add_learning_goal(
        description="Improve response accuracy for weather queries", priority=8
    )
