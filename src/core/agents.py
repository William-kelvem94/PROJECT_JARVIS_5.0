...
async def entrypoint(ctx: agents.JobContext):
    ...
    async def shutdown_hook():
        ...
        for item in chat_ctx.items:
            ...
            # Filter out system prompts and memory injections
    ...
    # Carrega memórias da KB para o agente (se configurada)
    try:
        from backend.app.config import settings
        from backend.app.local_memory import local_memory
                    ...
                    if filename.lower().endswith((".md", ".txt")):
                        ...
                        try:
                            with open(filepath, "r", encoding="utf-8") as fd:
                                preview = fd.read(800).replace("\n", " ").strip()
                                kb_summaries.append({"file": filename, "preview": preview})
                        except Exception:
                            continue

                    if len(kb_summaries) >= 5:
                        break
                if len(kb_summaries) >= 5:
                    break

            if kb_summaries:
                kb_str = json.dumps(kb_summaries, ensure_ascii=False)
                initial_ctx.add_message(
                    role="system",
                    content=(
                        f"KB de {kb_path} carregada. "
                        f"Arquivos iniciais e resumos: {kb_str}"
                    )
                )
                logger.info(f"[KB] Injetados no agente.")
        elif kb_path:
            logger.warning(f"[KB] Caminho configurado mas não existe: {kb_path}")
    except Exception as kb_err:
        logger.warning(f"[KB] Erro ao injetar KB: {kb_err}")
    ...
    async def watchdog_loop():
        from backend.app.utils.workflow_engine import workflow_engine
            ...
            try:
                if ctx.room.connection_state == "connected":
                    ...
                    for alert in alerts:
                        ...
                        from backend.app.utils.log_manager import log_manager
