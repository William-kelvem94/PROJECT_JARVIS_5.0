        ...
        for log_file in log_files:
            try:
                ...
                for block in traceback_blocks[1:]: # Skip preamble
                    ...
                    # Find the last file reference that belongs to OUR project (src/)
                    culprit = None
    ...
    def _attempt_fix(self, error_info: Dict) -> bool:
        ...
        try:
            ...
            logger.info(f"Asking Brain for fix on {file_path.name}...")
             ...
             # Create a new event loop for this sync call if needed, or use existing
            try:
            ...
            if loop.is_running():
                ...
                fixed_code_raw = asyncio.run(ai_agent._call_ollama_async(prompt, model="deepseek-r1:14b"))
            else:
                fixed_code_raw = loop.run_until_complete(ai_agent._call_ollama_async(prompt, model="deepseek-r1:14b"))
            ...
            logger.info(f"✅ APPLIED FIX to {file_path.name}. Backup at {backup_path.name}")
            return True
