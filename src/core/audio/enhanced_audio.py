...
def _import_torch():
    ...
    if not TORCH_AVAILABLE and torch is None:
        try:
            ...
            TORCH_AVAILABLE = False
            logger.warning(
                f"⚠️ torch not available - advanced audio features disabled: {e}"
            )
            torch = None
            torchaudio = None
...
try:
    ...
    logger.warning(
        f"âš ï¸ resemblyzer not available - speaker verification disabled: {e}"
    )
...
try:
    ...
    logger.warning(
        "âš ï¸ Wake Word detection (Porcupine) disabled - requirements missing"
    )
...
class EnhancedAudioSystem:
    ...
    def __init__(self, data_dir: Optional[Path] = None, event_bus=None):
        ...
        try:
            ...
            self.noise_reduction_enabled = _config.get_setting(
        ...
        self.auto_unload_timeout = (
            600  # 10 minutes for audio (less critical than vision)
        )
        ...
        logger.info(
            f"   Faster-Whisper: {'âœ…' if FASTER_WHISPER_AVAILABLE else 'âŒ'}"
        )
        logger.info(f"   PyAudio: {'âœ…' if PYAUDIO_AVAILABLE else 'âŒ'}")
        logger.info(
            f"   Speaker Verification: {'âœ…' if RESEMBLYZER_AVAILABLE else 'âŒ'}"
        )
    ...
    def start_background_loading(self):
        ...
        try:
            ...
            if mem.percent > 85:
                logger.warning(
                    f"⚠️ AudioSystem: High RAM usage ({mem.percent}%) - Delaying heavy model loading"
                )
        ...
        threading.Thread(
            target=self._load_models_background, daemon=True, name="AudioNeuralLoad"
        ).start()
    ...
    def _load_models_background(self):
            ...
            try:
                ...
                try:
                    with self._models_lock:
                        logger.info(
                            "🧠 Audio Core: Carregando Silero-VAD (Prioridade 1)..."
                        )
                        ...
                        if isinstance(result, tuple):
                            self.vad_model, self.vad_utils = result[0], (
        ...
        try:
            ...
            if mem.percent > 85.0:
                logger.warning(
                    f"⚠️ AudioSystem: High RAM ({mem.percent}%) - Skipping heavy Whisper load to prevent crash."
                )
                return
        except Exception as e:
        ...
        if FASTER_WHISPER_AVAILABLE:
            try:
                ...
                try:
                    with self._models_lock:
                        logger.info(
                            f"🧠 Audio Core: Carregando Faster-Whisper ({self.whisper_model_size})..."
                        )
                        ...
                        try:
                            if torch.cuda.is_available():
                                ...
                                logger.info(
                                    "   🚀 GPU NVIDIA Detectada: Usando CUDA + Float16"
                                )
                            else:
                                device = "cpu"
                                compute_type = "int8"
                                logger.info(
                                    "   💻 CPU Detectada: Usando INT8 para economia de memória"
                                )
                        except Exception:
                            device = "cpu"
                            compute_type = "int8"
                        ...
                        logger.info(
                            f"✅ Audio Core: Whisper pronto (Backend: {device.upper()}, Type: {compute_type})"
                        )
                finally:
                    model_load_lock.release()
            except Exception as e:
    ...
    def wait_for_models(self, timeout: float = 30.0) -> bool:
        ...
        logger.warning(
            "âš ï¸ Timeout waiting for audio models - system may be degraded"
        )
    ...
    def _update_activity(self):
        ...
        self._unload_timer = threading.Timer(
            self.auto_unload_timeout, self._auto_unload_models
        )
    ...
    def _auto_unload_models(self):
        ...
        ).total_seconds() >= self.auto_unload_timeout:
    ...
    def calibrate_vad_threshold(self, duration: float = 3.0):
        ...
        try:
            ...
            if ambient_samples:
                ...
                logger.info(
                    f"ðŸ“Š RuÃ­do ambiente medido: RMS mÃ©dio = {ambient_rms_mean:.1f}, std = {ambient_rms_std:.1f}"
                )
                logger.info(
    ...
    def start_listening(self):
        ...
        try:
            ...
            self._process_thread = threading.Thread(
                target=self._audio_processing_loop, daemon=True, name="AudioProcessing"
            )
    ...
    def _audio_processing_loop(self):
        ...
        audio_buffer = []
        silence_threshold = config.get_setting(
            "audio.silence_timeout", 1.0
        )  # Reduzido de 2.0s para 1.0s (Fase 5)
            ...
            try:
                ...
                if self.vad_model and len(audio_buffer) > 10:
                        ...
                        if self.is_awake or not self.wake_word_active:
                            ...
                            if len(audio_buffer) < 15:  # ~300ms de Ã¡udio
                                ...
                                if rms > 5000:  # Grito ou comando muito alto/claro
                                    logger.debug(
                                        "âš¡ Fast-Path: Disparando processamento imediato por energia alta"
                                    )
                                    self._process_audio_buffer(audio_buffer)
                                    audio_buffer = []
                                    voice_detected_in_buffer = False
                    ...
                    if elapsed_silence > silence_threshold and len(audio_buffer) > 0:
                        if (
                            voice_detected_in_buffer
                            and voice_frame_count >= MIN_VOICE_FRAMES
                        ):
                            self._process_audio_buffer(audio_buffer)
                    ...
                    if len(audio_buffer) > max_buffer_chunks:
                        logger.warning(
                            "âš ï¸ Buffer de Ã¡udio excedeu 30s, forÃ§ando flush"
                        )
                        if (
                            voice_detected_in_buffer
                            and voice_frame_count >= MIN_VOICE_FRAMES
    ...
    def _process_audio_buffer(self, audio_buffer: List[np.ndarray]):
        ...
        try:
            ...
            if self.voice_encoder and self.known_speakers:
                ...
                if speaker_verified:
                    logger.info(
                        f"âœ… Speaker verified: {speaker_id} ({confidence:.2f})"
                    )
                    ...
                    logger.warning(
                        f"âš ï¸ Unknown speaker detected (confidence: {confidence:.2f})"
                    )
            else:
            ...
            if self.whisper_model and speaker_verified:
                ...
                if self.event_bus:
                    try:
                        ...
                        logger.debug(
                            f"📣 Published transcription event: {result.text[:50]}..."
                        )
                    except Exception as e:
                ...
                if self.event_bus:
                    try:
                        ...
                        logger.warning(
                            f"\u26a0\ufe0f Failed to publish transcription event: {e}"
                        )
    ...
    def _transcribe_audio(self, audio: np.ndarray) -> TranscriptionResult:
        ...
        if not FASTER_WHISPER_AVAILABLE or not self.whisper_model:
            return TranscriptionResult(
                text="", language="pt", confidence=0.0, segments=[], processing_time=0.0
            )
        ...
        try:
            ...
            avg_confidence = (
                np.mean([s["confidence"] for s in segment_list])
                if segment_list
                else 0.0
            )
            ...
            return TranscriptionResult(
                text="", language="en", confidence=0.0, segments=[], processing_time=0.0
            )
...
if __name__ == "__main__":
    ...
    def on_transcription(result: TranscriptionResult):
        ...
        print(
            f"   Speaker: {result.speaker_id if result.speaker_verified else 'Unknown'}"
        )
