    ...
    def finish(self):
        ...
        if self.root_span_id in self.spans:
            ...
            if root_span.end_time:
                self.total_duration_ms = (root_span.end_time - root_span.start_time) * 1000
...
class TraceManager:
    ...
    def finish_span(self, span_id: str):
        ...
        if span:
            span.finish()
            logger.debug(f"Finished span: {span_id} - {span.name} ({span.duration_ms:.2f}ms)")
        else:
    ...
    def finish_trace(self, trace_id: str):
        ...
        if trace:
            ...
            span_ids_to_remove = [sid for sid, span in self.active_spans.items()
    ...
    @asynccontextmanager
    async def span_context(self, name: str, parent_span_id: str = None,
                          attributes: Dict[str, Any] = None):
        """
