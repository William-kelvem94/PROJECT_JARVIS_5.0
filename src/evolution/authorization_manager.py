...
class HumanAuthorizationManager:
    ...
    async def authorize(self, request_id: str, reason: Optional[str] = None) -> bool:
        ...
        if request.status != AuthorizationStatus.PENDING:
            logger.warning(
                f"Request {request_id} already processed: {request.status.value}"
            )
            return False
        ...
        if len(self.completed_requests) > self.max_completed_history:
            self.completed_requests = self.completed_requests[
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        rejected = sum(
            1
            for r in self.completed_requests
            if r.status == AuthorizationStatus.REJECTED
        )
        expired = sum(
            1
            for r in self.completed_requests
            if r.status == AuthorizationStatus.EXPIRED
        )
        ...
        return {
            ...
            "expired": expired,
            "approval_rate": (
                (approved / len(self.completed_requests) * 100)
