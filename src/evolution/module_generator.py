...
class ModuleGenerator:
    ...
    async def _handle_generation_request(self, event):
        ...
        if any(
            word in command
            for word in ["aprenda", "crie módulo", "learn", "create module"]
        ):
    ...
    async def generate_module(
        self, specification: str, source: str = "user"
        ...
        try:
            ...
            logger.info(
                f"✅ Module {module_name} generated and activated (monitoring for 24h)"
            )

            # Publish success event
            event_bus.publish(
                EventType.SYSTEM_STARTUP,
                data={"type": "module_generated", "module": module.to_dict()},
...
"""
    ...
    async def _generate_code(
        self, specification: str, module_name: str
    ) -> Optional[str]:
        """
...
'''
            ...
            try:
                ...
                result = subprocess.run(
                    [
                        sys.executable,
                        "-c",
                        f"import importlib.util; spec = importlib.util.spec_from_file_location('test_module', '{test_file}'); module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)",
                    ],
                    capture_output=True,
                    text=True,
                    timeout=10,
                ...
                for module_id, module in list(self.generated_modules.items()):
                    if module.status == ModuleStatus.MONITORING:
                        ...
                        if now > module.monitoring_until:
                            ...
                            logger.info(
                                f"✅ Module {module.name} monitoring complete - now ACTIVE"
                            )
                        ...
                        if module.error_count > 3:
                            ...
                            logger.warning(
                                f"⚠️ Module {module.name} disabled due to errors"
                            )
    ...
    def report_module_error(self, module_id: str, error: str):
        ...
        if module:
            ...
            logger.warning(
                f"⚠️ Module {module.name} error #{module.error_count}: {error}"
            )
    ...
    def get_active_modules(self) -> List[Dict[str, Any]]:
        ...
        return [
            m.to_dict()
            for m in self.generated_modules.values()
            if m.status
            in [ModuleStatus.ACTIVE, ModuleStatus.EXPERIMENTAL, ModuleStatus.MONITORING]
        ]
