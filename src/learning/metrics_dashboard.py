...
class MetricsCollector:
            ...
            if "torch" in self.system_collectors:
                ...
                for gpu_id in range(torch.cuda.device_count()):
                    try:
                        ...
                        gpu_metrics[gpu_id] = {
                            "memory_used_percent": (memory_allocated / memory_total)
                            * 100,
                            "memory_reserved_percent": (memory_reserved / memory_total)
                            * 100,
    ...
    def add_training_metric(
        ...
        metric_point = MetricPoint(
            timestamp=time.time(), value=value, job_id=job_id, metadata=safe_metadata
        )
    ...
    def get_latest_system_metrics(self) -> Optional[SystemMetrics]:
        ...
        with self._lock:
            if (
                "system" not in self.metrics_history
                or not self.metrics_history["system"]
            ):
                return None
...
class MetricsPersistence:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_training_job_metric ON training_metrics(job_id, metric_name)"
                )
                conn.execute(
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """,
                    (timestamp, job_id, metric_name, value, json.dumps(metadata or {})),
                )
                conn.commit()
    ...
    def get_training_metrics_history(
        self, job_id: str, metric_name: str, hours: int = 24
    ) -> List[MetricPoint]:
    ...
    def _init_flask_app(self):
        ...
        try:
            ...
            def get_training_metrics(job_id, metric_name):
                hours = request.args.get("hours", default=1, type=int)
                metrics = self.persistence.get_training_metrics_history(
                    job_id, metric_name, hours
                )
                return jsonify(
                    [
                        {
...
class MetricsDashboard:
    ...
    def start(self):
        ...
        logger.info(
            f"Metrics dashboard available at http://localhost:{self.web_dashboard.port}"
        )
    ...
    def add_training_metric(
        ...
        timestamp = time.time()
        self.persistence.save_training_metric(
            job_id, metric_name, value, timestamp, safe_metadata
        )
