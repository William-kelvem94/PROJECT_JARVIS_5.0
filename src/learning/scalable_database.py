    ...
    def create_tables(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_feedback_value ON feedback(feedback_value)"
                )
        ...
        try:
            ...
            try:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_feedback_timestamp_btree ON feedback USING btree(timestamp)"
                    )
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_feedback_type_hash ON feedback USING hash(feedback_type)"
                    )
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_feedback_value_btree ON feedback USING btree(feedback_value)"
                    )
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_feedback_metadata_gin ON feedback USING gin(metadata)"
                    )
                    cursor.execute(
                        "CREATE INDEX IF NOT EXISTS idx_feedback_text_gin ON feedback USING gin(to_tsvector('english', user_input || ' ' || ai_response))"
                    )
...
class VectorDatabase(DatabaseInterface):
    ...
    def __init__(self, db_path: Path):
        ...
        if not dependency_manager.is_available("database", "vector_db"):
            raise ImportError(
                "Vector database support requires chromadb: pip install chromadb"
            )
    ...
    def add_feedback(
        ...
        try:
            ...
            # Add to collection
            self.collection.add(
                documents=[document], ids=[feedback_id], metadatas=[meta]
            )
    ...
    def generate_preference_pairs(self, n_pairs: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, doc1 in enumerate(all_feedback["documents"][: n_pairs * 2]):
                ...
                similar = self.collection.query(query_texts=[doc1], n_results=5)

                for j, doc2 in enumerate(
                    similar["documents"][0][1:]
                ):  # Skip first (same doc)
...
class ScalableDatabase:
    ...
    def _initialize_backend(self):
        ...
        if sqlite_path.exists():
            try:
                ...
                if (
                    count > self.config.auto_migrate_threshold
                    and dependency_manager.is_available("database", "postgresql")
                ):
                    logger.info(
                        f"ðŸ”„ Auto-migrating to PostgreSQL ({count:,} records)"
                    )
    ...
    def get_backend_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "record_count": count,
            "can_migrate": (
                backend_type == "SQLiteDatabase"
                and count > self.config.auto_migrate_threshold
                and dependency_manager.is_available("database", "postgresql")
            ),
