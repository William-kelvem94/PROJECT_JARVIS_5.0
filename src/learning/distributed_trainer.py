...
class GPUManager:
    ...
    def allocate_gpus(self, job_id: str, num_gpus: int) -> List[int]:
        ...
        with self._lock:
            # Find available GPUs
            allocated_flat = [gpu for gpu_list in self.allocated_gpus.values() 
                            for gpu in gpu_list]
            available = [gpu for gpu in self.available_gpus if gpu not in allocated_flat]
            
            if len(available) < num_gpus:
                raise RuntimeError(f"Not enough GPUs available. Requested: {num_gpus}, Available: {len(available)}")
        ...
        for gpu_id in self.available_gpus:
            try:
                ...
                utilization[gpu_id] = {
                    ...
                    'allocated_to': next((job for job, gpus in self.allocated_gpus.items() 
                                        if gpu_id in gpus), None)
                }
...
class DistributedTrainer:
    ...
    def create_training_job(self,
                          ...
                          dataset_path: str,
                          output_dir: str,
                          num_gpus: Optional[int] = None) -> str:
            ...
            if len(job.gpu_allocation) > 1 and HAS_TORCH_DIST:
                ...
                try:
                    ...
                    if spawn_func:
                        ...
                        logger.warning("torch.multiprocessing.spawn not available, using single GPU")
        ...
        try:
            ...
            if HAS_TORCH_DIST:
                dist.init_process_group(
                    ...
                    rank=rank,
                    timeout=getattr(torch, 'distributed', {}).get('default_store_timeout') if hasattr(torch, 'distributed') else None
                )
            
            logger.info(f"Initialized worker {rank} on GPU {gpu_id} for job {job.job_id}")
            
            # Load and setup model
            model = self._create_distributed_model(job.model_config, rank)
            
            # Load dataset with distributed sampler
            train_loader = self._create_distributed_dataloader(job.dataset_path, rank, len(job.gpu_allocation))
    ...
    def _create_distributed_model(self, model_config: Dict[str, Any], rank: int):
        ...
        try:
            ...
            # Initialize trainer
            trainer = LocalTrainer(config=config, output_dir=Path(model_config.get('output_dir', '/tmp')))
    ...
    def _create_distributed_dataloader(self, dataset_path: str, rank: int, world_size: int):
    ...
    def _distributed_training_loop(self, model, train_loader, job: TrainingJob, rank: int):
            ...
            for epoch in range(5):  # Configurable
                ...
                for batch_idx, (data, targets) in enumerate(train_loader):
                    ...
                    if rank == 0 and batch_idx % 10 == 0:
                        logger.info(f"Job {job.job_id} Epoch {epoch}, Batch {batch_idx}, Loss: {loss.item():.4f}")
                
                # Log epoch metrics (only on rank 0)
                if rank == 0:
                    avg_loss = total_loss / num_batches
                    job.metrics[f'epoch_{epoch}_loss'] = avg_loss
                    logger.info(f"Job {job.job_id} Epoch {epoch} completed, Avg Loss: {avg_loss:.4f}")
