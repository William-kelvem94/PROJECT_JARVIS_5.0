...
class TopologyScanner:
    ...
    def scan_project(self) -> Dict[str, Any]:
        ...
        try:
            for root, dirs, files in os.walk(self.root):
                ...
                if depth > 3:
                    continue  # Limitar profundidade para performance
            ...
            logger.info(
                f"ðŸ“Š Topologia mapeada: {len(topology['structure'])} diretÃ³rios, Techs: {topology['tech_stack']}"
            )
            return topology
    ...
    def _detect_tech(self, file: str, ext: str, stack: set):
        if ext == ".py":
            ...
            stack.add("Python Pip")
        elif file == "Dockerfile":
            stack.add("Docker")
        elif ext == ".sql":
            stack.add("SQL Database")
