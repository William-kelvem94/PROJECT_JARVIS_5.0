...
class DependencyManager:
    ...
    def _check_openvino(self):
        ...
        try:
            ...
            self._status["openvino"] = DependencyStatus(
                available=False, error=str(e), capabilities=set()
            )
    ...
    def _check_torch(self):
        ...
        try:
            ...
            self._status["torch"] = DependencyStatus(
                available=True, version=torch.__version__, capabilities=capabilities
            )
            logger.debug(
                f"âœ… PyTorch {torch.__version__} available with capabilities: {capabilities}"
            )
        except Exception as e:
            self._status["torch"] = DependencyStatus(
                available=False,
...
>>>>>>> dev-new-version
        ...
        self._status["audio"] = DependencyStatus(
            available=len(capabilities) > 0, capabilities=capabilities
        )
...
>>>>>>> dev-new-version
    ...
    def is_available(self, dependency: str, capability: Optional[str] = None) -> bool:
        ...
        if status.fallback_available:
            logger.warning(
                f"Optional dependency '{dependency}' not installed; using fallback capabilities."
            )
            return True

        # Dependency is missing and no fallback is available
        logger.warning(
            f"Dependency '{dependency}' is missing. This may degrade functionality."
        )
        return False
    ...
    def get_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: {
                ...
                "capabilities": (
                    list(status.capabilities) if status.capabilities else []
                ),
    ...
    def ensure_capability(
        self, dependency: str, capability: str, error_msg: Optional[str] = None
    ):
        ...
        if not self.is_available(dependency, capability):
            if error_msg:
                ...
                raise ImportError(
                    f"Required capability '{capability}' not available for '{dependency}'"
                )
