...
from .jarvis_logger import setup_jarvis_logging
...
class LoggingConfig:
    ...
    def setup_timed_rotating_logger(
        ...
        # Formato de mensagem
        formatter = logging.Formatter(
            LoggingConfig.DEFAULT_FORMAT, datefmt=LoggingConfig.DEFAULT_DATE_FORMAT
        )
...
def get_or_create_logger(name: str, log_file: Optional[Path] = None) -> logging.Logger:
    ...
    return LoggingConfig.setup_rotating_logger(
        logger_name=name, log_file=log_file, level=logging.INFO
    )
