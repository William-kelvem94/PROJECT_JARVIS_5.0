...
async def emit_log(message: str, level: str = "INFO"):
    ...
    if WEB_SERVER_AVAILABLE and broadcast_message is not None:
        await broadcast_message({"type": "log", "message": message, "level": level})
    # 2. Desktop HUD
...
async def emit_telemetry(cpu: float, memory: float):
    ...
    # 1. Web
    if WEB_SERVER_AVAILABLE and broadcast_message is not None:
        await broadcast_message({"type": "telemetry", **data})
    # 2. Desktop HUD
    _notify_subscribers("telemetry", data)


async def emit_status(
    status: str, details: str = "", model: str = None, tier: str = "balanced"
):
    """
    Novo: Emite mudan̤a de status cognitivo
    Ex: status="thinking", details="Analisando logs...", model="llama3", tier="ultra"
    """
    data = {"status": status, "details": details, "model": model, "tier": tier}
    # 1. Web
    if WEB_SERVER_AVAILABLE and broadcast_message is not None:
        await broadcast_message({"type": "status", **data})
    # 2. Desktop HUD
