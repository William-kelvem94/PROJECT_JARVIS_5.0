...
class FileIndexer:
    ...
    def start_background_indexing(self, search_paths: List[str]):
        ...
        thread = threading.Thread(target=self._indexing_loop, args=(search_paths,), daemon=True, name="FileIndexer")
        thread.start()
    ...
    def _indexing_loop(self, search_paths: List[str]):
        ...
        for path_str in search_paths:
            ...
            for file in path.rglob("*"):
                if not self.is_indexing: break # Permite parar o processamento
                ...
                try:
                    ...
                    if str(file) in self.index and self.index[str(file)]["mtime"] == mtime:
                        continue
        ...
        self.save_cache()
        self.is_indexing = False
        logger.info(f"✅ Indexação adaptativa concluída. Itens no cache: {len(self.index)}")
    ...
    def _extract_metadata(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if ext in ['.txt', '.py', '.md', '.json', '.yaml', '.log', '.csv', '.xml'] or (mime and mime.startswith('text/')):
            ...
            # ESTRATÉGIA B: PDFs e Office
                ...
                meta['summary'] = f"Documento Office ({ext}). Metadados serão extraídos em análise profunda."
