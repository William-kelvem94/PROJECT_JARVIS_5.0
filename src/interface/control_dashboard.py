...
=======
            if urls and any(
                url.toLocalFile().endswith((".json", ".yaml", ".yml")) for url in urls
            ):
...
class ControlDashboard(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        logo_label.setStyleSheet(
            "font-size: 16px; font-weight: bold; letter-spacing: 5px; color: #00F2FF; padding-left: 20px;"
        )
        sub_logo = QLabel("CONTROL ACCESS // v5.0")
        sub_logo.setStyleSheet(
            "font-size: 8px; color: rgba(255, 255, 255, 0.4); padding-left: 22px; letter-spacing: 2px;"
        )
        ...
        self.quick_cpu.setStyleSheet(
            "font-size: 10px; color: rgba(255,255,255,0.5); font-family: 'Consolas';"
        )
    ...
    def _create_brain_tab(self) -> QWidget:
        ...
        self.model_type_combo.addItems(
            ["Ollama (Local)", "OpenAI", "Anthropic", "Google Gemini", "Groq"]
        )
        ...
        self.model_name_edit.setText(
            self.config.get("ai", {}).get("model_name", "llama3")
        )
    ...
    def _create_voice_tab(self) -> QWidget:
        ...
        self.stt_model_combo.addItems(
            ["faster-whisper", "openai-whisper", "vosk", "google"]
        )
        ...
        self.voice_combo.addItems(
            ["en-US-AriaNeural", "en-US-GuyNeural", "pt-BR-FranciscaNeural"]
        )
    ...
    def _create_config_tab(self) -> QWidget:
        ...
        title.setStyleSheet(
            "font-size: 18px; font-weight: bold; color: #00F2FF; margin-bottom: 10px;"
        )
        layout.addWidget(title)
        ...
        self.config_path_label = QLabel(
            "No file loaded - Drag & drop config files here"
        )
    ...
    def _create_learning_tab(self) -> QWidget:
        ...
        self.learning_status_label.setStyleSheet(
            "color: #00F2FF; font-size: 18px; font-weight: bold;"
        )
        sc_layout.addWidget(self.learning_status_label)

        self.cog_awareness_label = QLabel("COGNITIVE AWARENESS: MONITORING")
        self.cog_awareness_label.setStyleSheet(
            "color: #7000FF; font-size: 12px; font-weight: bold;"
        )
        ...
        tg_layout.addWidget(
            QLabel("Force immediate neural weights update or backup current state.")
        )
        ...
        self.cog_research_btn.setStyleSheet(
            "background: rgba(112, 0, 255, 0.1); border: 1px solid #7000FF;"
        )
        ...
        self.correction_edit.setPlaceholderText(
            "Provide the 'Golden Response' to correct behavior..."
        )
    ...
    def _refresh_learning_status(self):
        ...
        try:
            ...
            if engine and engine.is_initialized:
                ...
                # Update metrics
                self.total_interactions_label.setText(
                    str(status.get("total_feedback", 0))
                )
                self.training_cycles_label.setText(
                    str(status.get("training_cycles", 0))
                )

            else:
                self.learning_status_label.setText(
                    "âŒ OFFLINE - Learning systems not initialized"
                )
        ...
        try:
            ...
            if engine and engine.knowledge_distiller:
                ...
                for cmd_key, data in list(golden_cmds.items())[:20]:  # Show top 20
                    ...
                    self.golden_list.addItem(
                        f"[{usage}x] {data.get('command', cmd_key)}"
                    )
...
=======
                if (
                    hasattr(engine, "last_interaction_id")
                    and engine.last_interaction_id
                ):
>>>>>>> dev-new-version
                    engine.record_explicit_feedback(
                        interaction_id=engine.last_interaction_id, feedback_value=value
                    )
                    ...
                    logger.info(
                        f"User feedback: {value} for interaction {engine.last_interaction_id}"
                    )
                else:
                    QMessageBox.warning(
                        self, "No Interaction", "No recent interaction found to rate."
                    )
            else:
    ...
    def _submit_correction(self):
        ...
        if not correction:
            QMessageBox.warning(
                self, "Empty Correction", "Please provide a correction first."
            )
            return
        ...
        try:
            ...
            if engine:
                if (
                    ...
                    logger.info(
                        f"User correction: {correction} for interaction {engine.last_interaction_id}"
                    )
                else:
                    QMessageBox.warning(
                        self,
                        "No Interaction",
                        "No recent interaction found to correct.",
                    )
            else:
                QMessageBox.warning(self, "Error", "Learning engine not available")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                if engine and engine.continual_learner:
                    ...
                    QMessageBox.warning(
                        self, "Error", "Continual learner not available"
                    )
    ...
    def _backup_model(self):
        ...
        try:
            ...
            if not source.exists():
                QMessageBox.warning(
                    self, "No Model", "No trained model found to backup."
                )
                return
            ...
            QMessageBox.information(
                self, "Backup Complete", f"Model backed up to:\n{backup_path}"
            )
    ...
    def _refresh_processes(self):
        ...
        try:
            ...
            for proc in all_procs:
                ...
                # RAM
                memory_mb = info.get("memory_info", type("obj", (), {"rss": 0})).rss / (
                    1024 * 1024
                )
                self.process_table.setItem(
                    row, 3, QTableWidgetItem(f"{memory_mb:.1f} MB")
                )
...
<<<<<<< Updated upstream
    ...
    def _request_hud_mode(self):
        ...
        try:
            ...
            logger.error(
                "Could not import InterfaceMode. WindowManager might not be in path."
            )
    ...
    def _load_selected_config(self):
        ...
        if not current_item:
            QMessageBox.warning(
                self, "No Selection", "Please select a config file first."
            )
            return
    ...
    def _save_current_config(self):
        ...
        try:
            ...
            show_success_toast(
                "Config Saved", "Configuration file updated successfully"
            )
    ...
    def _format_config_json(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Invalid JSON", f"Cannot format invalid JSON: {e}"
            )
    ...
    def _restore_config_backup(self):
        ...
        backup_files = list(
            self.current_config_path.parent.glob(
                f"{self.current_config_path.stem}.backup_*.json"
            )
        )
