...
class ArcReactorWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.color_cyan = JarvisTheme.get_color_with_alpha(
            JarvisTheme.PRIMARY_CYAN, JarvisTheme.ALPHA_HIGH
        )
        self.color_orange = JarvisTheme.get_color_with_alpha(
            JarvisTheme.SECONDARY_ORANGE, JarvisTheme.ALPHA_HIGH
        )
    ...
    def _animate(self):
        ...
        self.spark_angle = (
            self.spark_angle + (speed * 4 if self.is_studying else speed)
        ) % 360
        if self.isVisible():
            self.update()
