...
class TestSecurityManagerIsolated(unittest.TestCase):
    ...
    def setUp(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "security_manager", project_root / "src/core/security/security_manager.py"
        )
...
class TestIOTManagerIsolated(unittest.TestCase):
    ...
    def setUp(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "iot_manager", project_root / "src/core/iot/iot_manager.py"
        )
...
class TestStarkOrchestratorIsolated(unittest.TestCase):
    ...
    def setUp(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "orchestrator", project_root / "src/core/orchestrator.py"
        )
    ...
    def test_component_management(self):
        ...
        test_components = {"security": Mock(), "iot": Mock(), "fallback": Mock()}
...
class TestSystemIntegration(unittest.TestCase):
    ...
    def test_complete_workflow_simulation(self):
        ...
        self.assertFalse(
            security.validate_path_access("C:\\Windows\\System32\\evil.exe")
        )
        ...
        for submodule in submodules:
            ...
            if init_file.exists():
                ...
                if content.strip() != "# clean" and len(content.strip()) > 10:
                    ...
                    if '"""' in content:
                        print(
                            f"   ✅ src/core/{submodule}/__init__.py melhorado e documentado"
                        )
                    else:
                        print(
                            f"   ⚠️ src/core/{submodule}/__init__.py melhorado mas sem doc"
                        )

        self.assertGreater(
            improved_count,
            0,
            "Pelo menos um submodule deveria ter __init__.py melhorado",
        )
...
def run_all_tests():
    ...
    for test_class in test_classes:
        ...
        if result.wasSuccessful():
            print(
                f"   ✅ {test_class.__name__}: {class_passed}/{class_tests} testes passaram"
            )
        else:
            print(
                f"   ❌ {test_class.__name__}: {class_passed}/{class_tests} testes passaram"
