...
class DeviceManagerTester:
    ...
    def test_list_interfaces(self) -> bool:
        ...
        for iface in interfaces[:3]:  # Mostra primeiras 3
            print(f"  - {iface['name']}: UP={iface['is_up']} Speed={iface.get('speed_mbps')} Mbps")
    ...
    def test_audio_control(self) -> bool:
        ...
        print(f"Volume original: {original_volume * 100:.0f}%" if original_volume else "Não disponível")
    ...
    def test_display_info(self) -> bool:
        ...
        for display in displays:
            print(f"  - {display.get('name')}: {display.get('screen_width')}x{display.get('screen_height')}")
    ...
    def test_list_processes(self) -> bool:
        ...
        for proc in processes[:5]:
            print(f"  - {proc.get('name')} (PID: {proc.get('pid')}): CPU={proc.get('cpu_percent'):.1f}% RAM={proc.get('memory_percent'):.1f}%")
    ...
    def test_registry_write_safe(self) -> bool:
        ...
        if success:
            ...
            print(f"✅ Escrita e leitura do registro OK")
    ...
    def test_list_services(self) -> bool:
        ...
        if len(services) > 0:
            ...
            print(f"Serviços em execução (amostra):")
            for svc in running:
