...
class TestConfig:
    ...
    @patch(
        "src.utils.config.open", new_callable=mock_open, read_data='{"test": "data"}'
    )
    def test_load_user_settings_success(self, mock_file):
        ...
        mock_file.assert_called_once_with(
            config_obj.SETTINGS_FILE, "r", encoding="utf-8"
        )
    ...
    @patch(
        "src.utils.config.open", side_effect=json.JSONDecodeError("Invalid JSON", "", 0)
    )
    ...
    def test_save_user_settings(self, mock_json_dump, mock_file):
        ...
        mock_json_dump.assert_called_once_with(
            test_settings, mock_file.return_value, indent=2, ensure_ascii=False
        )

        # Verificar se arquivo foi aberto corretamente
        mock_file.assert_called_once_with(
            config_obj.SETTINGS_FILE, "w", encoding="utf-8"
        )
