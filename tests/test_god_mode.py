...
print(f"   RAM: {hw_info['memory']['used_gb']}GB / {hw_info['memory']['total_gb']}GB ({hw_info['memory']['percent']}%)")
print(f"   Disco: {hw_info['disk']['used_gb']}GB / {hw_info['disk']['total_gb']}GB ({hw_info['disk']['percent']}%)")
if hw_info['bios']:
    print(f"   BIOS: {hw_info['bios'].get('manufacturer', 'N/A')} - {hw_info['bios'].get('version', 'N/A')}")

# Test 3: Process List (top 5)
print("\n3️⃣ TOP 5 PROCESSOS (CPU):")
processes = system_controller.list_processes()
sorted_procs = sorted(processes, key=lambda x: x.get('cpu_percent', 0), reverse=True)[:5]
for proc in sorted_procs:
    print(f"   {proc['name']}: {proc['cpu_percent']}% CPU, {proc.get('memory_percent', 0):.1f}% RAM")
