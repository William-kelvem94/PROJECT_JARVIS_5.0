...
def test_2_ai_config_loading():
    ...
    try:
        ...
        ollama_url = config.get_ai_config(
...
def test_3_dependency_verification():
    ...
    try:
        ...
        print("✅ AIAgent criado")
        print(f"✅ safe_mode = {agent.safe_mode}")

        if agent.safe_mode:
            print("⚠️ Sistema em MODO SEGURO - dependências críticas faltando")
            print(
                "💡 Isso é esperado se screen_capture ou action_controller não estiverem disponíveis"
            )
        else:
        ...
        assert hasattr(
            agent, "_verify_critical_dependencies"
...
def test_4_brain_router_config():
    ...
    try:
        ...
        assert hasattr(
            brain_router, "_choose_local_brain"
